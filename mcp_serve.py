"""
WHOX MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (WHOX-specific extra)

Usage:
    whox mcp serve
    whox mcp serve --verbose
...
logger = logging.getLogger("whox.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using WHOX_HOME."""
    try:
        from whox_constants import get_whox_home
        return get_whox_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("WHOX_HOME", Path.home() / ".whox")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from whox_constants import get_whox_home
        directory_file = get_whox_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("WHOX_HOME", Path.home() / ".whox")
...
class EventBridge:
    ...
    This is the WHOX equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from whox_constants import get_whox_home
            db_file = get_whox_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("WHOX_HOME", Path.home() / ".whox")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the WHOX MCP server with all tools registered."""
    if not _MCP_SERVER_AVAILABLE:
        raise ImportError(
            "MCP server requires the 'mcp' package. "
            "Install with: pip install 'whox-agent[mcp]'"
        )

    mcp = FastMCP(
        "whox",
        instructions=(
            "WHOX Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the WHOX MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
