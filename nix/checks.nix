...
{ inputs, ... }: {
  perSystem = { pkgs, system, lib, ... }:
    let
      whox-agent = inputs.self.packages.${system}.default;
      ...
      configKeys = pkgs.runCommand "whox-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${whoxVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = lib.optionalAttrs pkgs.stdenv.hostPlatform.isLinux {
        ...
        package-contents = pkgs.runCommand "whox-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${whox-agent}/bin/whox || (echo "FAIL: whox binary missing"; exit 1)
          test -x ${whox-agent}/bin/whox-agent || (echo "FAIL: whox-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${whox-agent}/bin/whox version 2>&1 | grep -qi "whox" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "whox-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in whox whox-agent whox-acp; do
            test -x ${whox-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "whox-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking whox --help ==="
          ${whox-agent}/bin/whox --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${whox-agent}/bin/whox --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "whox-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${whox-agent}/share/whox-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${whox-agent}/share/whox-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "WHOX_BUNDLED_SKILLS" ${whox-agent}/bin/whox || \
            (echo "FAIL: WHOX_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: WHOX_BUNDLED_SKILLS set in wrapper"
        ...
        # Verify WHOX_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "whox-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(WHOX_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking WHOX_MANAGED guards ==="
          check_blocked "config set" ${whox-agent}/bin/whox config set model foo
          check_blocked "config edit" ${whox-agent}/bin/whox config edit
        ...
        in pkgs.runCommand "whox-config-roundtrip" {
          ...
          merge_and_load() {
            local whox_home="$1"
            export WHOX_HOME="$whox_home"
            ${configMergeScript} ${nixSettings} "$whox_home/config.yaml"
            ${whoxVenv}/bin/python3 -c '
import json, sys
