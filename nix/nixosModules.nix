# nix/nixosModules.nix — NixOS module for whox-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: whox runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $WHOX_HOME/.env
# and read by whox at startup — no container recreation needed for env changes.
#
# Tool resolution: the whox wrapper uses --suffix PATH for nix store tools,
...
#     environmentFiles = [ config.sops.secrets."whox/env".path ];
#   };
#
{ inputs, ... }: {
  ...
  let
    cfg = config.services.whox-agent;
    whox-agent = inputs.self.packages.${pkgs.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-whox-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "whox-config-attrs";
      description = "WHOX YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "whox-config.yaml" configJson;
    ...
    documentDerivation = pkgs.runCommand "whox-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'WHOX_DOC_EOF'\n${value}\nWHOX_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "whox-agent";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/whox";
    ...
    # whox user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "whox-container-entrypoint" ''
      set -eu

      WHOX_UID="''${WHOX_UID:?WHOX_UID must be set}"
      WHOX_GID="''${WHOX_GID:?WHOX_GID must be set}"

      # ── Group: ensure a group with GID=$WHOX_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$WHOX_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="whox"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$WHOX_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$WHOX_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$WHOX_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$WHOX_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="whox"
        TARGET_HOME="/home/whox"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$WHOX_UID" -g "$WHOX_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$WHOX_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$WHOX_UID:$WHOX_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure WHOX_HOME is owned by the target user
      if [ -n "''${WHOX_HOME:-}" ] && [ -d "$WHOX_HOME" ]; then
        chown -R "$WHOX_UID:$WHOX_GID" "$WHOX_HOME"
      fi
      ...
      if [ ! -f /var/lib/whox-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        echo "First boot: provisioning agent tools..."
        apt-get update -qq
        apt-get install -y -qq sudo nodejs npm curl
        touch /var/lib/whox-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/whox ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/whox
        chmod 0440 /etc/sudoers.d/whox
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$WHOX_UID" --regid="$WHOX_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $WHOX_HOME/.env.
    ...
    # Default: /var/lib/whox/workspace → /data/workspace.
  ...
  in {
    options.services.whox-agent = with lib; {
      enable = mkEnableOption "WHOX Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = whox-agent;
        description = "The whox-agent package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/whox";
        description = "State directory. Contains .whox/ subdir (WHOX_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative WHOX config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $WHOX_HOME/.env at activation time.
          WHOX reads this file on every startup via load_whox_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $WHOX_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $WHOX_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `whox gateway`.";
      };
      ...
      addToSystemPackages = mkOption {
        type = types.bool;
        default = false;
        description = "Add whox CLI to environment.systemPackages.";
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.whox-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.whox        0750 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."whox-agent-setup" = lib.stringAfter [ "users" ] ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.whox
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.whox ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 0750 ${cfg.stateDir} ${cfg.stateDir}/.whox ${cfg.stateDir}/home ${cfg.workingDirectory}

          # Merge Nix settings into existing config.yaml.
          # Preserves user-added keys (skills, streaming, etc.); Nix keys win.
          # If configFile is user-provided (not generated), overwrite instead of merge.
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m 0640 -D ${configFile} ${cfg.stateDir}/.whox/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.whox/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.whox/config.yaml
            chmod 0640 ${cfg.stateDir}/.whox/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.whox/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.whox/.managed
          chmod 0644 ${cfg.stateDir}/.whox/.managed

          # Seed auth file if provided
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.whox/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.whox/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.whox/auth.json
              fi
            ''}
          ''}
          ...
          # WHOX reads $WHOX_HOME/.env at startup via load_whox_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.whox/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0600 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'WHOX_NIX_ENV_EOF'
${envFileContent}
WHOX_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.whox-agent = {
          description = "WHOX Agent Gateway";
          ...
          serviceConfig = {
            ...
            # $WHOX_HOME/.env by the activation script. load_whox_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${cfg.package}/bin/whox"
              "gateway"
            ] ++ cfg.extraArgs);
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.whox-agent = {
          description = "WHOX Agent Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              WHOX_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              WHOX_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env WHOX_UID="$WHOX_UID" \
                --env WHOX_GID="$WHOX_GID" \
                --env WHOX_HOME=${containerDataDir}/.whox \
                --env WHOX_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/whox gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
