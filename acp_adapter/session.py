"""ACP session manager — maps ACP sessions to WHOX AIAgent instances.

Sessions are persisted to the shared SessionDB (``~/.whox/state.db``) so they
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed WHOX agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by WHOX AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current WHOX runtime provider configuration.
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.whox/state.db``) is lazily created.
        """
    ...
    def _get_db(self):
        ...
        Note: we resolve ``WHOX_HOME`` dynamically rather than relying on
        ...
        later (e.g. by the test fixture ``_isolate_whox_home``).
        """
        ...
        try:
            ...
            self._db_instance = SessionDB(db_path=whox_home / "state.db")
            return self._db_instance
        except Exception:
