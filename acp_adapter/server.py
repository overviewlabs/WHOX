"""ACP agent server — exposes WHOX Agent via the Agent Client Protocol."""
...
try:
    from whox_cli import __version__ as WHOX_VERSION
except Exception:
    WHOX_VERSION = "0.0.0"
...
class WHOXACPAgent(acp.Agent):
    """ACP Agent implementation wrapping WHOX AIAgent."""
    ...
    async def _register_session_mcp_servers(
        ...
        try:
            ...
            enabled_toolsets = getattr(state.agent, "enabled_toolsets", None) or ["whox-acp"]
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        if provider:
            auth_methods = [
                AuthMethod(
                    ...
                    description=f"Authenticate WHOX using the currently configured {provider} runtime credentials.",
                )
            ]
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="whox-agent", version=WHOX_VERSION),
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        """Run WHOX on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
    ...
    def _cmd_model(self, args: str, state: SessionState) -> str:
        ...
        try:
            from whox_cli.models import parse_model_input, detect_provider_for_model
    ...
    def _cmd_tools(self, args: str, state: SessionState) -> str:
        try:
            from model_tools import get_tool_definitions
            toolsets = getattr(state.agent, "enabled_toolsets", None) or ["whox-acp"]
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"WHOX Agent v{WHOX_VERSION}"
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when WHOX has no typed ACP config surface yet."""
        state = self.session_manager.get_session(session_id)
        if state is None:
