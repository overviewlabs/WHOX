...
# Configuration
REPO_URL_SSH="git@github.com:NousResearch/whox-agent.git"
REPO_URL_HTTPS="https://github.com/NousResearch/whox-agent.git"
WHOX_HOME="$HOME/.whox"
INSTALL_DIR="${WHOX_INSTALL_DIR:-$WHOX_HOME/whox-agent}"
PYTHON_VERSION="3.11"
NODE_VERSION="22"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "  --dir PATH     Installation directory (default: ~/.whox/whox-agent)"
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  irm https://raw.githubusercontent.com/NousResearch/whox-agent/main/scripts/install.ps1 | iex"
            exit 1
            ;;
        *)
            OS="unknown"
...
check_node() {
    ...
    if [ -x "$WHOX_HOME/node/bin/node" ]; then
        export PATH="$WHOX_HOME/node/bin:$PATH"
        local found_ver=$("$WHOX_HOME/node/bin/node" --version)
        log_success "Node.js $found_ver found (WHOX-managed)"
        HAS_NODE=true
        return 0
    fi
...
install_node() {
    ...
    log_info "Extracting to ~/.whox/node/..."
    ...
    # Place into ~/.whox/node/ and symlink binaries to ~/.local/bin/
    rm -rf "$WHOX_HOME/node"
    mkdir -p "$WHOX_HOME"
    mv "$extracted_dir" "$WHOX_HOME/node"
    rm -rf "$tmp_dir"

    mkdir -p "$HOME/.local/bin"
    ln -sf "$WHOX_HOME/node/bin/node" "$HOME/.local/bin/node"
    ln -sf "$WHOX_HOME/node/bin/npm"  "$HOME/.local/bin/npm"
    ln -sf "$WHOX_HOME/node/bin/npx"  "$HOME/.local/bin/npx"

    export PATH="$WHOX_HOME/node/bin:$PATH"

    local installed_ver
    installed_ver=$("$WHOX_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.whox/node/"
    HAS_NODE=true
}
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "WHOX Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                local stash_name
                stash_name="whox-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if WHOX behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up whox command..."

    if [ "$USE_VENV" = true ]; then
        WHOX_BIN="$INSTALL_DIR/venv/bin/whox"
    else
        WHOX_BIN="$(which whox 2>/dev/null || echo "")"
        if [ -z "$WHOX_BIN" ]; then
            log_warn "whox not found on PATH after install"
            return 0
        fi
    fi
    ...
    if [ ! -x "$WHOX_BIN" ]; then
        log_warn "whox entry point not found at $WHOX_BIN"
    ...
    mkdir -p "$HOME/.local/bin"
    ln -sf "$WHOX_BIN" "$HOME/.local/bin/whox"
    log_success "Symlinked whox → ~/.local/bin/whox"
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$HOME/.local/bin$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# WHOX Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
    ...
    # Export for current session so whox works immediately
    export PATH="$HOME/.local/bin:$PATH"

    log_success "whox command ready"
}

copy_config_templates() {
    log_info "Setting up configuration files..."

    # Create ~/.whox directory structure (config at top level, code in subdir)
    mkdir -p "$WHOX_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills,whatsapp/session}

    # Create .env at ~/.whox/.env (top level, easy to find)
    if [ ! -f "$WHOX_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/config/examples/.env.example" ]; then
            ...
            log_success "Created ~/.whox/.env from template"
        elif [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$WHOX_HOME/.env"
            log_success "Created ~/.whox/.env from template"
        else
            touch "$WHOX_HOME/.env"
            log_success "Created ~/.whox/.env"
        fi
    else
        log_info "~/.whox/.env already exists, keeping it"
    fi

    # Create config.yaml at ~/.whox/config.yaml (top level, easy to find)
...
-->
SOUL_EOF
        log_success "Created ~/.whox/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.whox/"

    # Seed bundled skills into ~/.whox/skills/ (manifest-based, one-time per skill)
    log_info "Syncing bundled skills to ~/.whox/skills/ ..."
    if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
        log_success "Skills synced to ~/.whox/skills/"
    else
        # Fallback: simple directory copy if Python sync fails
        if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$WHOX_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
            cp -r "$INSTALL_DIR/skills/"* "$WHOX_HOME/skills/" 2>/dev/null || true
            log_success "Skills copied to ~/.whox/skills/"
        fi
    fi
}
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        case "$DISTRO" in
            ...
            *)
                ...
                log_info "This is standard Playwright setup — WHOX itself does not require root access."
...
run_setup_wizard() {
    ...
    if ! [ -e /dev/tty ]; then
        log_info "Setup wizard skipped (no terminal available). Run 'whox setup' after install."
        return 0
    fi
    ...
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m whox_cli.main setup < /dev/tty
    else
        python -m whox_cli.main setup < /dev/tty
    fi
}
...
maybe_start_gateway() {
    ...
    log_info "The gateway needs to be running for WHOX to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$WHOX_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'whox whatsapp' to pair via QR code..."
            echo ""
            read -p "Pair WhatsApp now? [Y/n] " -n 1 -r
            echo
            if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
                WHOX_CMD="$HOME/.local/bin/whox"
                [ ! -x "$WHOX_CMD" ] && WHOX_CMD="whox"
                $WHOX_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'whox whatsapp' to pair."
        fi
    fi

    if ! [ -e /dev/tty ]; then
        log_info "Gateway setup skipped (no terminal available). Run 'whox gateway install' later."
        return 0
    fi
    ...
    if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
        ...
        if command -v systemctl &> /dev/null; then
            ...
            if $WHOX_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $WHOX_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: whox gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: whox gateway"
            fi
        else
            log_info "systemd not available — starting gateway in background..."
            nohup $WHOX_CMD gateway > "$WHOX_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.whox/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: whox gateway"
        fi
    else
        log_info "Skipped. Start the gateway later with: whox gateway"
    fi
}
...
print_success() {
    ...
    # Show file locations
    echo -e "${CYAN}${BOLD}📁 Your files (all in ~/.whox/):${NC}"
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    ~/.whox/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  ~/.whox/.env"
    echo -e "   ${YELLOW}Data:${NC}      ~/.whox/cron/, sessions/, logs/"
    echo -e "   ${YELLOW}Code:${NC}      ~/.whox/whox-agent/"
    echo ""
    ...
    echo ""
    echo -e "   ${GREEN}whox${NC}              Start chatting"
    echo -e "   ${GREEN}whox setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}whox config${NC}       View/edit configuration"
    echo -e "   ${GREEN}whox config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}whox gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}whox update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    echo -e "${YELLOW}⚡ Reload your shell to use 'whox' command:${NC}"
    echo ""
    LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
