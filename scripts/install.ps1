# ============================================================================
# WHOX Agent Installer for Windows
# ============================================================================
# Installation script for Windows (PowerShell).
# Uses uv for fast Python provisioning and package management.
#
# Usage:
#   irm https://raw.githubusercontent.com/NousResearch/whox-agent/main/scripts/install.ps1 | iex
#
...
param(
    ...
    [switch]$SkipSetup,
    [string]$Branch = "main",
    [string]$WHOXHome = "$env:LOCALAPPDATA\whox",
    [string]$InstallDir = "$env:LOCALAPPDATA\whox\whox-agent"
)
...
$RepoUrlSsh = "git@github.com:NousResearch/whox-agent.git"
$RepoUrlHttps = "https://github.com/NousResearch/whox-agent.git"
$PythonVersion = "3.11"
$NodeVersion = "22"
...
function Write-Banner {
    ...
    Write-Host "│             ⚕ WHOX Agent Installer                   │" -ForegroundColor Magenta
    Write-Host "├─────────────────────────────────────────────────────────┤" -ForegroundColor Magenta
...
function Test-Node {
    ...
    $managedNode = "$WHOXHome\node\node.exe"
    if (Test-Path $managedNode) {
        $version = & $managedNode --version
        $env:Path = "$WHOXHome\node;$env:Path"
        Write-Success "Node.js $version found (WHOX-managed)"
        $script:HasNode = $true
        return $true
    }
    ...
    # Fallback: download binary zip to ~/.whox/node/
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\whox-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$WHOXHome\node") { Remove-Item -Recurse -Force "$WHOXHome\node" }
                Move-Item $extractedDir.FullName "$WHOXHome\node"
                $env:Path = "$WHOXHome\node;$env:Path"

                $version = & "$WHOXHome\node\node.exe" --version
                Write-Success "Node.js $version installed to ~/.whox/node/"
                $script:HasNode = $true
...
function Install-Repository {
    ...
    if (Test-Path $InstallDir) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                $zipUrl = "https://github.com/NousResearch/whox-agent/archive/refs/heads/$Branch.zip"
                $zipPath = "$env:TEMP\whox-agent-$Branch.zip"
                $extractPath = "$env:TEMP\whox-agent-extract"
...
function Set-PathVariable {
    Write-Info "Setting up whox command..."
    
    if ($NoVenv) {
        $whoxBin = "$InstallDir"
    } else {
        $whoxBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so whox is globally available
    # On Windows, the whox.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$whoxBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$whoxBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $whoxBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set WHOX_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\whox instead
    # of the Unix default ~/.whox
    $currentWHOXHome = [Environment]::GetEnvironmentVariable("WHOX_HOME", "User")
    if (-not $currentWHOXHome -or $currentWHOXHome -ne $WHOXHome) {
        [Environment]::SetEnvironmentVariable("WHOX_HOME", $WHOXHome, "User")
        Write-Success "Set WHOX_HOME=$WHOXHome"
    }
    $env:WHOX_HOME = $WHOXHome
    
    # Update current session
    $env:Path = "$whoxBin;$env:Path"
    
    Write-Success "whox command ready"
}

function Copy-ConfigTemplates {
    Write-Info "Setting up configuration files..."
    
    # Create ~/.whox directory structure
    New-Item -ItemType Directory -Force -Path "$WHOXHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$WHOXHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$WHOXHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$WHOXHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$WHOXHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$WHOXHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$WHOXHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$WHOXHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$WHOXHome\skills" | Out-Null
    New-Item -ItemType Directory -Force -Path "$WHOXHome\whatsapp\session" | Out-Null
    
    # Create .env
    $envPath = "$WHOXHome\.env"
    if (-not (Test-Path $envPath)) {
        $examplePathNew = "$InstallDir\config\examples\.env.example"
        $examplePathOld = "$InstallDir\.env.example"
        if (Test-Path $examplePathNew) {
            Copy-Item $examplePathNew $envPath
            Write-Success "Created ~/.whox/.env from template"
        } elseif (Test-Path $examplePathOld) {
            Copy-Item $examplePathOld $envPath
            Write-Success "Created ~/.whox/.env from template"
        } else {
            New-Item -ItemType File -Force -Path $envPath | Out-Null
            Write-Success "Created ~/.whox/.env"
        }
    } else {
        Write-Info "~/.whox/.env already exists, keeping it"
    }
    
    # Create config.yaml
    $configPath = "$WHOXHome\config.yaml"
    if (-not (Test-Path $configPath)) {
        $examplePathNew = "$InstallDir\config\examples\cli-config.yaml.example"
        $examplePathOld = "$InstallDir\cli-config.yaml.example"
        if (Test-Path $examplePathNew) {
            Copy-Item $examplePathNew $configPath
            Write-Success "Created ~/.whox/config.yaml from template"
        } elseif (Test-Path $examplePathOld) {
            Copy-Item $examplePathOld $configPath
            Write-Success "Created ~/.whox/config.yaml from template"
        }
    } else {
        Write-Info "~/.whox/config.yaml already exists, keeping it"
    }
    
    # Create SOUL.md if it doesn't exist (global persona file)
    $soulPath = "$WHOXHome\SOUL.md"
...
Edit this to customize how WHOX communicates with you.
...
"@ | Set-Content -Path $soulPath -Encoding UTF8
        Write-Success "Created ~/.whox/SOUL.md (edit to customize personality)"
    }
    
    Write-Success "Configuration directory ready: ~/.whox/"
    
    # Seed bundled skills into ~/.whox/skills/ (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to ~/.whox/skills/ ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to ~/.whox/skills/"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$WHOXHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to ~/.whox/skills/"
            }
        }
    }
}
...
function Invoke-SetupWizard {
    ...
    # Run whox setup using the venv Python directly (no activation needed)
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m whox_cli.main setup
    } else {
        python -m whox_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $whoxCmd = "$InstallDir\venv\Scripts\whox.exe"
    if (-not (Test-Path $whoxCmd)) {
        $whoxCmd = "whox"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$WHOXHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'whox whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$WHOXHome\logs\gateway.log"
            Start-Process -FilePath $whoxCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$WHOXHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: whox gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: whox gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$WHOXHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$WHOXHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$WHOXHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$WHOXHome\whox-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   whox              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   whox setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   whox config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   whox config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   whox gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   whox update       " -NoNewline -ForegroundColor Green
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/NousResearch/whox-agent/main/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
