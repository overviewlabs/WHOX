...
if tinker_atropos_dir.exists():
    ...
    os.environ['WHOX_QUIET'] = '1'  # Disable temp subdirectory creation
    print(f"📂 Terminal working directory: {tinker_atropos_dir}")
else:
    # Fall back to whox-agent directory if submodule not found
...
from whox_constants import get_whox_home, OPENROUTER_BASE_URL

DEFAULT_MODEL = "anthropic/claude-opus-4.5"
DEFAULT_BASE_URL = OPENROUTER_BASE_URL


def load_whox_config() -> dict:
    """
...
def main(
    ...
    Args:
        ...
        model: Model to use for the agent (reads from ~/.whox/config.yaml if not provided)
    ...
    """
    # Load config from ~/.whox/config.yaml
    config = load_whox_config()
