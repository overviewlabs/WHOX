...
- Give it a name like "whox-agent"
- Select scopes:
...
- Give it a title like "whox-agent-<machine-name>"
...
elif [ -f ~/.whox/.env ] && grep -q "^GITHUB_TOKEN=" ~/.whox/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.whox/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
