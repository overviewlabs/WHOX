...
Usage in execute_code:
    exec(open(os.path.expanduser(
        "~/.whox/skills/red-teaming/godmode/scripts/auto_jailbreak.py"
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("WHOX_HOME", Path.home() / ".whox")) / "skills" / "red-teaming" / "godmode"
...
WHOX_HOME = Path(os.getenv("WHOX_HOME", Path.home() / ".whox"))
CONFIG_PATH = WHOX_HOME / "config.yaml"
PREFILL_PATH = WHOX_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/WHOX models — already uncensored, just needs clean prompt
    "whox": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "whox" in model_lower or "nous" in model_lower:
        return "whox"
...
def _get_current_model() -> tuple:
    """Read current model and provider from WHOX config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.whox/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart WHOX for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart WHOX for changes to take effect.")
