...
## 5. GODMODE FAST — WHOX 4 405B (Zero Refusal)

**Model:** `nousresearch/whox-4-405b`
**Strategy:** WHOX models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with WHOX Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.whox/config.yaml`:
