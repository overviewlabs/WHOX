#!/usr/bin/env python3
"""Google Workspace API CLI for WHOX Agent.
...
def get_credentials():
    ...
    if missing_scopes:
        ...
        print(
            f"Re-run setup.py from the active WHOX profile ({display_whox_home()}) to restore full access.",
            file=sys.stderr,
        )
        sys.exit(1)
    return creds
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for WHOX Agent")
