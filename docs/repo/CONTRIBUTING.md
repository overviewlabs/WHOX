# Contributing to WHOX Agent

Thank you for contributing to WHOX Agent! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — Windows, macOS, different Linux distros, different terminal emulators. We want WHOX to work everywhere.
...
Bundled skills (in `skills/`) ship with every WHOX install. They should be **broadly useful to most users**:

- Document handling, web research, common dev workflows, system administration
- Used regularly by a wide range of people

If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `whox skills browse` (labeled "official") and install it with `whox skills install` (no third-party warning, builtin trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Nous Research Discord](https://discord.gg/NousResearch). Users can install it with `whox skills install`.
...
```bash
git clone --recurse-submodules https://github.com/NousResearch/whox-agent.git
cd whox-agent
...
```bash
mkdir -p ~/.whox/{cron,sessions,logs,memories,skills}
cp config/examples/cli-config.yaml.example ~/.whox/config.yaml
touch ~/.whox/.env

# Add at minimum an LLM provider key:
echo 'OPENROUTER_API_KEY=sk-or-v1-your-key' >> ~/.whox/.env
```
...
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/whox" ~/.local/bin/whox
...
├── cli.py                    # WHOXCLI class — interactive TUI, prompt_toolkit integration
├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
├── toolsets.py               # Tool groupings and presets (whox-cli, whox-telegram, etc.)
├── whox_state.py           # SQLite session database with FTS5 full-text search, session titles
...
│
├── whox_cli/               # CLI command implementations
...
│
├── skills/                   # Bundled skills (copied to ~/.whox/skills/ on install)
├── optional-skills/          # Official optional skills (discoverable via hub, not activated by default)
├── environments/             # RL training environments (Atropos integration)
├── tests/                    # Test suite
├── website/                  # Documentation site (whox-agent.nousresearch.com)
│
├── config/examples/cli-config.yaml.example   # Example configuration (copied to ~/.whox/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.whox/`)

| Path | Purpose |
|------|---------|
| `~/.whox/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.whox/.env` | API keys and secrets |
| `~/.whox/auth.json` | OAuth credentials (Nous Portal) |
| `~/.whox/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.whox/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.whox/state.db` | SQLite session database |
| `~/.whox/sessions/` | JSON session logs |
| `~/.whox/cron/` | Scheduled job data |
| `~/.whox/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`whox_state.py`) with full-text search and unique session titles. JSON logs go to `~/.whox/sessions/`.
...
Four fields are supported under `metadata.whox`:
...
The user may skip setup and keep loading the skill. WHOX only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `whox setup` or update `~/.whox/.env` locally.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing WHOX tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `whox --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

WHOX uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.whox/skins/<name>.yaml`:
...
Add to `_BUILTIN_SKINS` dict in `whox_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `whox_cli/skin_engine.py` for the full schema and existing skins as examples.

---

## Cross-Platform Compatibility

WHOX runs on Linux, macOS, and Windows. When writing code that touches the OS:
...
WHOX has terminal access. Security matters.
...
2. **Test manually**: Run `whox` and exercise the code path you changed
...
- Use [GitHub Issues](https://github.com/NousResearch/whox-agent/issues)
- Include: OS, Python version, WHOX version (`whox version`), full error traceback
