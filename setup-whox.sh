...
# 5. Symlinks the CLI commands into ~/.local/bin (whox + whox)
# 6. Runs the setup wizard (optional)
# ============================================================================

set -e

WHOX_NONINTERACTIVE="${WHOX_NONINTERACTIVE:-0}"
...
if [ ! -f ".env" ]; then
    if [ -f "$SCRIPT_DIR/config/examples/.env.example" ]; then
        cp "$SCRIPT_DIR/config/examples/.env.example" .env
...
# ============================================================================
# PATH setup — symlink CLI commands into ~/.local/bin
# ============================================================================

echo -e "${CYAN}→${NC} Setting up WHOX commands..."

WHOX_BIN="$SCRIPT_DIR/venv/bin/whox"
mkdir -p "$HOME/.local/bin"
ln -sf "$WHOX_BIN" "$HOME/.local/bin/whox"
ln -sf "$WHOX_BIN" "$HOME/.local/bin/whox"
echo -e "${GREEN}✓${NC} Symlinked whox → ~/.local/bin/whox"
echo -e "${GREEN}✓${NC} Symlinked whox → ~/.local/bin/whox"
...
if [ -n "$SHELL_CONFIG" ]; then
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$HOME/.local/bin$"; then
        if ! grep -q '\.local/bin' "$SHELL_CONFIG" 2>/dev/null; then
            echo "" >> "$SHELL_CONFIG"
            echo "# WHOX — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
...
# ============================================================================
# Seed bundled skills into ~/.whox/skills/
# ============================================================================

WHOX_SKILLS_DIR="${WHOX_HOME:-$HOME/.whox}/skills"
mkdir -p "$WHOX_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.whox/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$WHOX_SKILLS_DIR/" 2>/dev/null || true
...
echo ""
echo -e "${GREEN}✓ WHOX setup complete!${NC}"
echo ""
echo "Next steps:"
echo ""
...
echo ""
echo "  2. Run the WHOX setup wizard (optional advanced config):"
echo "     whox setup"
echo ""
...
echo ""
echo "Other commands:"
echo "  whox status          # Check configuration"
echo "  whox gateway install # Install gateway service (messaging + cron)"
echo "  whox cron list       # View scheduled jobs"
echo "  whox doctor          # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z "${REPLY:-}" ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m whox_cli.main setup
fi
