"""Slash command definitions and autocomplete for the WHOX CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("paste", "Check clipboard for an image and attach it", "Info",
               cli_only=True),
    CommandDef("update", "Update WHOX Agent to the latest version", "Info",
               gateway_only=True),
...
def _resolve_config_gates() -> set[str]:
    ...
    try:
        ...
        config_path = os.path.join(
            os.getenv("WHOX_HOME", os.path.expanduser("~/.whox")),
            "config.yaml",
        )
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``whox skills
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /whox handler.

    Maps both canonical names and aliases so /whox bg do stuff works
    the same as /whox background do stuff.
    """
    overrides = _resolve_config_gates()
