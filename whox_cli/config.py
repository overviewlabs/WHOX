"""
Configuration management for WHOX Agent.

Config files are stored in ~/.whox/ for easy access:
- ~/.whox/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.whox/.env         - API keys and secrets

This module provides:
- whox config          - Show current configuration
- whox config edit     - Open config in editor
- whox config set      - Set a specific value
- whox config wizard   - Re-run setup wizard
"""
...
from whox_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("WHOX_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_whox_home() / ".managed"
...
def is_managed() -> bool:
    """Check if WHOX is running in package-manager-managed mode.

    Two signals: the WHOX_MANAGED env var (set by the systemd service),
    or a .managed marker file in WHOX_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade whox-agent"
...
def recommended_update_command() -> str:
    ...
    return get_managed_update_command() or "whox update"


def format_managed_message(action: str = "modify this WHOX installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("WHOX_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this WHOX installation is managed by NixOS "
            f"(WHOX_MANAGED={env_hint}).\n"
            "Edit services.whox-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this WHOX installation is managed by Homebrew "
            f"(WHOX_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade whox-agent"
        )

    return (
        f"Cannot {action}: this WHOX installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall WHOX."
    )
...
# Re-export from whox_constants — canonical definition lives there.
from whox_constants import get_whox_home  # noqa: F811,E402

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_whox_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_whox_home() / ".env"
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into WHOX_HOME if the user doesn't have one yet."""
...
def ensure_whox_home():
    """Ensure ~/.whox directory structure exists with secure permissions."""
    home = get_whox_home()
...
DEFAULT_CONFIG = {
    ...
    "terminal": {
        ...
        # docker_env lets you specify exact key-value pairs — useful when WHOX
    ...
    "browser": {
        ...
        "camofox": {
            # When true, WHOX sends a stable profile-scoped userId to Camofox
    ...
    # always goes to ~/.whox/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
    },

    # Honcho AI-native memory -- reads ~/.honcho/config.json as single source of truth.
    # This section is only needed for whox-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *WHOX Agent*" header.
...
OPTIONAL_ENV_VARS = {
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the WHOX auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @whox:example.org)",
    ...
    # WHOX_TOOL_PROGRESS and WHOX_TOOL_PROGRESS_MODE are deprecated —
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("WHOX_TOOL_PROGRESS")
            old_mode = get_env_value("WHOX_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in ("false", "0", "no"):
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from WHOX_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in ("new", "all"):
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from WHOX_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("WHOX_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from WHOX_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in ("y", "yes"):
                ...
                print("  Set later with: whox config set <key> <value>")
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.whox/config.yaml."""
    import copy
    ensure_whox_home()
...
#   openai-codex (OAuth — whox login) — OpenAI Codex
#   nous         (OAuth — whox login) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.whox/config.yaml."""
    if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.whox/.env."""
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real WHOX env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.whox/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.whox/.env."""
    if is_managed():
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.whox/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ WHOX Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(color("  whox config edit     # Edit config file", Colors.DIM))
    print(color("  whox config set <key> <value>", Colors.DIM))
    print(color("  whox setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: whox config set <key> <value>")
            print()
            print("Examples:")
            print("  whox config set model anthropic/claude-sonnet-4")
            print("  whox config set terminal.backend docker")
            print("  whox config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'whox config migrate' to add them")
        ...
        print("  whox config           Show current configuration")
        print("  whox config edit      Open config in editor")
        print("  whox config set <key> <value>   Set a config value")
        print("  whox config check     Check for missing/outdated config")
        print("  whox config migrate   Update config with new options")
        print("  whox config path      Show config file path")
        print("  whox config env-path  Show .env file path")
        sys.exit(1)
