#!/usr/bin/env python3
"""
WHOX CLI - Main entry point.

Usage:
    whox                     # Interactive chat (default)
    whox chat                # Interactive chat
    whox gateway             # Run gateway in foreground
    whox gateway start       # Start gateway as service
    whox gateway stop        # Stop gateway service
    whox gateway status      # Show gateway status
    whox gateway install     # Install gateway service
    whox gateway uninstall   # Uninstall gateway service
    whox setup               # Interactive setup wizard
    whox logout              # Clear stored authentication
    whox status              # Show status of all components
    whox cron                # Manage cron jobs
    whox cron list           # List cron jobs
    whox cron status         # Check if cron scheduler is running
    whox doctor              # Check configuration and dependencies
    whox honcho setup                    # Configure Honcho AI memory integration
    whox honcho status                   # Show Honcho config and connection status
    whox honcho sessions                 # List directory → session name mappings
    whox honcho map <name>               # Map current directory to a session name
    whox honcho peer                     # Show peer names and dialectic settings
    whox honcho peer --user NAME         # Set user peer name
    whox honcho peer --ai NAME           # Set AI peer name
    whox honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    whox honcho mode                     # Show current memory mode
    whox honcho mode [hybrid|honcho|local]  # Set memory mode
    whox honcho tokens                   # Show token budget settings
    whox honcho tokens --context N       # Set session.context() token cap
    whox honcho tokens --dialectic N     # Set dialectic result char cap
    whox honcho identity                 # Show AI peer identity representation
    whox honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    whox honcho migrate                  # Step-by-step migration guide for Honcho memory modes
    whox version             Show version
    whox update              Update to latest version
    whox uninstall           Uninstall WHOX Agent
    whox acp                 Run as an ACP server for editor integration
    whox sessions browse     Interactive session picker with search

    whox claw migrate --dry-run  # Disabled in WHOX distribution
"""
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (whox tools, whox setup, whox model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'whox {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any whox module import.
#
# Many modules cache WHOX_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("WHOX_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.whox/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set WHOX_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    ...
    # 2. If no flag, check ~/.whox/active_profile
    if profile_name is None:
        try:
            active_path = Path.home() / ".whox" / "active_profile"
    ...
    # 3. If we found a profile, resolve and set WHOX_HOME
    if profile_name is not None:
        try:
            from whox_cli.profiles import resolve_profile_env
            whox_home = resolve_profile_env(profile_name)
        except (ValueError, FileNotFoundError) as exc:
            print(f"Error: {exc}", file=sys.stderr)
            sys.exit(1)
        except Exception as exc:
            # A bug in profiles.py must NEVER prevent whox from starting
            print(f"Warning: profile override failed ({exc}), using default", file=sys.stderr)
            return
        os.environ["WHOX_HOME"] = whox_home
...
# Load .env from ~/.whox/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from whox_cli.config import get_whox_home
from whox_cli.env_loader import load_whox_dotenv
load_whox_dotenv(project_env=PROJECT_ROOT / '.env')
...
from whox_cli import __version__, __release_date__
from whox_constants import OPENROUTER_BASE_URL
...
def _has_any_provider_configured() -> bool:
    ...
    from whox_cli.config import get_env_path, get_whox_home, load_config
    from whox_cli.auth import get_auth_status

    # Determine whether WHOX itself has been explicitly configured (model
    # in config that isn't the hardcoded default). Used below to gate external
    # tool credentials (Claude Code, Codex CLI) that shouldn't silently skip
    # the setup wizard on a fresh install.
    from whox_cli.config import DEFAULT_CONFIG
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_whox_config = _model_name and _model_name != _DEFAULT_MODEL

    # Check env vars (may be set by .env or shell).
    # OPENAI_BASE_URL alone counts — local models (vLLM, llama.cpp, etc.)
    # often don't require an API key.
    from whox_cli.auth import PROVIDER_REGISTRY
    ...
    auth_file = get_whox_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if WHOX has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants WHOX to use their tokens.
    if _has_whox_config:
        try:
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'whox sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    if not _has_any_provider_configured():
        print()
        print("It looks like WHOX isn't configured yet -- no API keys or providers found.")
        print()
        print("  Run:  whox setup")
        print()

        from whox_cli.setup import is_interactive_stdin, print_noninteractive_setup_guidance
        ...
        if reply in ("", "y", "yes"):
            ...
            return
        print()
        print("You can run 'whox setup' at any time to configure.")
        sys.exit(1)

    # Start update check in background (runs while other init happens)
    try:
        from whox_cli.banner import prefetch_update_check
    ...
    if getattr(args, "source", None):
        os.environ["WHOX_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from whox_cli.gateway import gateway_command
...
def cmd_whatsapp(args):
    ...
    import subprocess
    from pathlib import Path
    from whox_cli.config import get_env_value, save_env_value
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with WHOX?")
        print()
    ...
    session_dir = get_whox_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in ("y", "yes"):
            ...
            print("  Start the gateway with: whox gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  whox gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ WHOX Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  whox gateway")
        ...
        print()
        print("  Or install as a service: whox gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'whox whatsapp' to try again.")


def cmd_setup(args):
    """Interactive setup wizard."""
    _require_tty("setup")
    # Friendly alias: "features" maps to the existing tools configurator flow.
    if getattr(args, "section", None) == "features":
        args.section = "tools"
    from whox_cli.setup import run_setup_wizard
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``whox model``) and the setup wizard
    (``setup_model_provider`` in setup.py).  Handles the full flow:
    provider picker, credential prompting, model selection, and config
    persistence.
    """
    from whox_cli.auth import (
        resolve_provider, AuthError, format_auth_error,
    )
    from whox_cli.config import load_config, get_env_value
    ...
    effective_provider = (
        config_provider
        or os.getenv("WHOX_INFERENCE_PROVIDER")
        or "auto"
    )
    try:
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from whox_cli.auth import _prompt_model_selection, _save_model_choice, deactivate_provider
    from whox_cli.config import get_env_value, save_env_value
    ...
    from whox_cli.models import model_ids
    ...
    if selected:
        ...
        from whox_cli.config import load_config, save_config
        cfg = load_config()
        model = cfg.get("model")
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from whox_cli.config import get_env_value, save_config, save_env_value
    from whox_cli.nous_subscription import (
    ...
    from whox_cli.models import _PROVIDER_MODELS
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from whox_cli.codex_models import get_codex_model_ids
    from whox_cli.config import get_env_value, save_env_value
    import argparse
    ...
    _codex_token = None
    try:
        from whox_cli.auth import resolve_codex_runtime_credentials
...
def _model_flow_custom(config):
    ...
    """
    from whox_cli.auth import _save_model_choice, deactivate_provider
    from whox_cli.config import get_env_value, save_env_value, load_config, save_config
    ...
    from whox_cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `whox model` to set a model.")
...
def _model_flow_named_custom(config, provider_info):
    ...
    """
    from whox_cli.auth import _save_model_choice, deactivate_provider
    from whox_cli.config import save_env_value, load_config, save_config
    from whox_cli.models import fetch_api_models
...
def _model_flow_copilot(config, current_model=""):
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from whox_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from whox_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates WHOX turns to `copilot --acp`.")
    print("  WHOX currently starts its own ACP subprocess for each request.")
    print("  WHOX uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print("  Set WHOX_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere.")
        return
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    from whox_cli.config import get_env_value, save_env_value, load_config, save_config
    from whox_cli.models import fetch_api_models, opencode_model_api_mode, normalize_opencode_model_id
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from whox_constants import display_whox_home as _dhh_fn
            print(f"    WHOX will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env.")
            return True
        return False
...
def _model_flow_anthropic(config, current_model=""):
    ...
    if selected:
        ...
        # providers if the user switches without running 'whox model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    """Authenticate WHOX CLI with a provider."""
    from whox_cli.auth import login_command
    login_command(args)


def cmd_logout(args):
    """Clear provider authentication."""
    from whox_cli.auth import logout_command
    logout_command(args)


def cmd_auth(args):
    """Manage pooled credentials."""
    from whox_cli.auth_commands import auth_command
    auth_command(args)


def cmd_status(args):
    """Show status of all components."""
    from whox_cli.status import show_status
    show_status(args)


def cmd_cron(args):
    """Cron job management."""
    from whox_cli.cron import cron_command
    cron_command(args)


def cmd_webhook(args):
    """Webhook subscription management."""
    from whox_cli.webhook import webhook_command
    webhook_command(args)


def cmd_doctor(args):
    """Check configuration and dependencies."""
    from whox_cli.doctor import run_doctor
    run_doctor(args)


def cmd_config(args):
    """Configuration management."""
    from whox_cli.config import config_command
    config_command(args)


def cmd_version(args):
    """Show version."""
    print(f"WHOX Agent v{__version__} ({__release_date__})")
    ...
    try:
        from whox_cli.banner import check_for_updates
        from whox_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    """Uninstall WHOX Agent."""
    _require_tty("uninstall")
    from whox_cli.uninstall import run_uninstall
    run_uninstall(args)
...
def _update_via_zip(args):
    """Update WHOX Agent by downloading a ZIP archive.
    ...
    branch = "main"
    zip_url = f"https://github.com/NousResearch/whox-agent/archive/refs/heads/{branch}.zip"
    
    print("→ Downloading latest version...")
    try:
        tmp_dir = tempfile.mkdtemp(prefix="whox-update-")
        zip_path = os.path.join(tmp_dir, f"whox-agent-{branch}.zip")
        ...
        # GitHub ZIPs extract to whox-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"whox-agent-{branch}")
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime("whox-update-autostash-%Y%m%d-%H%M%S")
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if WHOX behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        do_reset = True
        if prompt_user:
            print("\nReset working tree to clean state so WHOX can run?")
    ...
    if stash_selector is None:
        print("⚠ Local changes were restored, but WHOX couldn't find the stash entry to drop.")
        ...
        if drop.returncode != 0:
            print("⚠ Local changes were restored, but WHOX couldn't drop the saved stash entry.")
    ...
    print("  Review `git diff` / `git status` if WHOX behaves unexpectedly.")
    return True

# =========================================================================
# Fork detection and upstream management for `whox update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/NousResearch/whox-agent.git",
    "git@github.com:NousResearch/whox-agent.git",
    "https://github.com/NousResearch/whox-agent",
    "git@github.com:NousResearch/whox-agent",
}
OFFICIAL_REPO_URL = "https://github.com/NousResearch/whox-agent.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_whox_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        from whox_constants import get_whox_home
        (get_whox_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official WHOX repository.")
        print("  This means you may miss updates from NousResearch/whox-agent.")
        print()
        try:
        ...
        if response in ("", "y", "yes"):
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print("  ✓ Added upstream: https://github.com/NousResearch/whox-agent.git")
                has_upstream = True
            else:
                print("  ✗ Failed to add upstream remote. Skipping upstream sync.")
                return
        else:
            print("  Skipped. Run 'git remote add upstream https://github.com/NousResearch/whox-agent.git' to add later.")
...
def _invalidate_update_cache():
    ...
    ``whox update``, every profile is now current.
    """
    homes = []
    # Default profile home
    default_home = Path.home() / ".whox"
    homes.append(default_home)
    # Named profiles under ~/.whox/profiles/
...
def _load_installable_optional_extras() -> list[str]:
    ...
    # Entries look like "whox-agent[matrix]" or "package-name[extra]".
...
def cmd_update(args):
    """Update WHOX Agent to the latest version."""
    import shutil
    from whox_cli.config import is_managed, managed_error

    if is_managed():
        managed_error("update WHOX Agent")
        return
    
    print("⚕ Updating WHOX Agent...")
    print()
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            print("  curl -fsSL https://raw.githubusercontent.com/NousResearch/whox-agent/main/scripts/install.sh | bash")
            sys.exit(1)
    ...
    try:
        ...
        # the old bytecode (e.g. get_whox_home added to whox_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}")
        ...
        # modules in this process.  Reload whox_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_whox_home() added since the last release.
        try:
            import importlib
            import whox_constants as _hc
        ...
        try:
            from whox_cli.profiles import list_profiles, get_active_profile_name, seed_profile_skills
        ...
        if needs_migration:
            ...
            if not (sys.stdin.isatty() and sys.stdout.isatty()):
                ...
                print("    Run 'whox config migrate' later to apply any new config/env options.")
                response = "n"
            else:
                try:
            ...
            if response in ('', 'y', 'yes'):
                ...
                print()
                print("Skipped. Run 'whox config migrate' later to configure.")
        else:
        ...
        try:
            ...
            # Discover all whox-gateway* units (default + profiles)
            if is_linux():
                try:
                ...
                for scope, scope_cmd in [("user", ["systemctl", "--user"]), ("system", ["systemctl"])]:
                    try:
                        result = subprocess.run(
                            scope_cmd + ["list-units", "whox-gateway*", "--plain", "--no-legend", "--no-pager"],
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            if not parts:
                                continue
                            unit = parts[0]  # e.g. whox-gateway.service or whox-gateway-coder.service
            ...
            if is_macos():
                try:
                    from whox_cli.gateway import launchd_restart, get_launchd_label, get_launchd_plist_path
            ...
            if restarted_services or killed_pids:
                ...
                if killed_pids:
                    ...
                    print("    Restart manually: whox gateway run")
                    # Also restart for each profile if needed
                    if len(killed_pids) > 1:
                        print("    (or: whox -p <profile> gateway run  for each profile)")
        ...
        print("  whox model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``whox -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    from whox_constants import display_whox_home

    action = getattr(args, "profile_action", None)

    if action is None:
        # Bare `whox profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_whox_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → whox -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.whox)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(f"  Choose a custom alias:  whox profile alias {name} --name <custom>")
                    print(f"  Or access via flag:     whox -p {name} chat")
                else:
            ...
            if clone or clone_all:
                from whox_constants import get_whox_home
                profile_dir_display = f"~/.whox/profiles/{name}"
        ...
        name = args.profile_name
        from whox_cli.profiles import get_profile_dir, profile_exists, _read_config_model, _check_gateway_running, _count_skills
        ...
        from whox_cli.profiles import profile_exists
        if not profile_exists(name):
            print(f"Error: Profile '{name}' does not exist.")
            sys.exit(1)
        ...
        from whox_cli.profiles import rename_profile
        try:
        ...
        from whox_cli.profiles import export_profile
        name = args.profile_name
        ...
        from whox_cli.profiles import import_profile
        try:
...
def cmd_completion(args):
    ...
    from whox_cli.profiles import generate_bash_completion, generate_zsh_completion
...
def main():
    """Main entry point for whox CLI."""
    parser = argparse.ArgumentParser(
        prog="whox",
        description="WHOX Assistant - WHOX is Where Humans Optimize...",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
    whox                        Start interactive chat
    whox chat -q "Hello"        Single query mode
    whox -c                     Resume the most recent session
    whox -c "my project"        Resume a session by name (latest in lineage)
    whox --resume <session_id>  Resume a specific session by ID
    whox setup                  Run setup wizard
    whox logout                 Clear stored authentication
    whox auth add <provider>    Add a pooled credential
    whox auth list              List pooled credentials
    whox auth remove <p> <n>    Remove pooled credential by index
    whox auth reset <provider>  Clear exhaustion status for a provider
    whox model                  Select default model
    whox config                 View configuration
    whox config edit            Edit config in $EDITOR
    whox config set model gpt-4 Set a config value
    whox gateway                Run messaging gateway
    whox -s whox-agent-dev,github-auth
    whox -w                     Start in isolated git worktree
    whox gateway install        Install gateway background service
    whox sessions list          List past sessions
    whox sessions browse        Interactive session picker
    whox sessions rename ID T   Rename/title a session
    whox update                 Update to latest version
...
"""
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with WHOX Agent"
    )
    ...
    model_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use for Nous login (default: whox-cli)"
    )
    model_parser.add_argument(
        "--scope",
        default=None,
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure WHOX Agent with an interactive wizard. "
                    "Run a specific section: whox setup model|terminal|gateway|tools|features|agent"
    )
    setup_parser.add_argument(
        "section",
        nargs="?",
        choices=["model", "terminal", "gateway", "tools", "features", "agent"],
        default=None,
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for WHOX CLI"
    )
    ...
    login_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use (default: whox-cli)"
    )
    login_parser.add_argument(
        "--scope",
        default=None,
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of WHOX Agent components"
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with WHOX Agent setup"
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage WHOX Agent configuration"
    )
    ...
    def cmd_pairing(args):
        from whox_cli.pairing import pairing_command
    ...
    def cmd_skills(args):
        ...
        if getattr(args, 'skills_action', None) == 'config':
            ...
            from whox_cli.skills_config import skills_command as skills_config_command
            skills_config_command(args)
        else:
            from whox_cli.skills_hub import skills_command
            skills_command(args)
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL or owner/repo shorthand (e.g. anpicasso/whox-plugin-chrome-profiles)",
    )
    plugins_install.add_argument(
    ...
    def cmd_plugins(args):
        from whox_cli.plugins_cmd import plugins_command
    ...
    # Provider selection happens via 'whox memory setup'.
    # =========================================================================
    honcho_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "plugin — initial setup is via 'whox memory setup'. These commands\n"
    ...
    honcho_subparsers.add_parser("setup", help="Initial Honcho setup (redirects to whox memory setup)")
    ...
    honcho_subparsers.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to WHOX Honcho",
    )
    ...
    def cmd_honcho(args):
        ...
        if sub == "setup":
            ...
            print("  Running 'whox memory setup'...\n")
            from whox_cli.memory_setup import memory_command
            memory_command(args)
            return
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'whox tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # whox tools disable <name...> [--platform cli]
    ...
    # whox tools enable <name...> [--platform cli]
    ...
    def cmd_tools(args):
        ...
        if action in ("list", "disable", "enable"):
            from whox_cli.tools_config import tools_disable_enable_command
            tools_disable_enable_command(args)
        else:
            _require_tty("tools")
            from whox_cli.tools_config import tools_command
            tools_command(args)
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run WHOX as an MCP server",
        description=(
            "Manage MCP server connections and run WHOX as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'whox mcp add' to connect to a new server, or\n"
            "'whox mcp serve' to expose WHOX conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run WHOX as an MCP server (expose conversations to other agents)",
    )
    ...
    def cmd_mcp(args):
        from whox_cli.mcp_config import mcp_command
        mcp_command(args)
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            # Launch whox --resume <id> by replacing the current process
            print(f"Resuming session: {selected_id}")
            import shutil
            whox_bin = shutil.which("whox")
            if whox_bin:
                os.execvp(whox_bin, ["whox", "--resume", selected_id])
            else:
                # Fallback: re-invoke via python -m
                os.execvp(
                    sys.executable,
                    [sys.executable, "-m", "whox_cli.main", "--resume", selected_id],
                )
    ...
    # =========================================================================
    # claw command (disabled in WHOX distribution)
    # =========================================================================
    claw_parser = subparsers.add_parser(
        "claw",
        help="Migration commands (disabled in WHOX)",
        description="OpenClaw migration commands are disabled in WHOX distribution"
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Disabled in WHOX",
        description="OpenClaw migration is disabled in WHOX distribution"
    )
    ...
    claw_cleanup = claw_subparsers.add_parser(
        ...
        aliases=["clean"],
        help="Disabled in WHOX",
        description="OpenClaw migration cleanup is disabled in WHOX distribution"
    )
    ...
    def cmd_claw(args):
        from whox_cli.claw import claw_command
        claw_command(args)
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update WHOX Agent to the latest version",
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall WHOX Agent",
        description="Remove WHOX Agent from your system. Can keep configs/data for reinstall."
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run WHOX Agent as an ACP (Agent Client Protocol) server",
        description="Start WHOX Agent in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )

    def cmd_acp(args):
        """Launch WHOX Agent as an ACP server."""
        try:
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated WHOX instances",
    )
    ...
    # e.g. ``whox -c Pokemon Agent Dev`` → ``whox -c 'Pokemon Agent Dev'``
