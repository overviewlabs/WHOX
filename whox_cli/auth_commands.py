...
import whox_cli.auth as auth_mod
...
def auth_add_command(args) -> None:
    ...
    if provider == "anthropic":
        ...
        creds = anthropic_mod.run_whox_oauth_login_pure()
        if not creds:
        ...
        entry = PooledCredential(
            ...
            priority=0,
            source=f"{SOURCE_MANUAL}:whox_pkce",
    ...
    raise SystemExit(f"`whox auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `whox auth` is called bare."""
