"""
Doctor command for whox CLI.

Diagnoses issues with WHOX Agent setup.
"""
...
from whox_cli.config import get_project_root, get_whox_home, get_env_path
from whox_constants import display_whox_home

PROJECT_ROOT = get_project_root()
WHOX_HOME = get_whox_home()
_DHH = display_whox_home()  # user-facing display path (e.g. ~/.whox or ~/.whox/profiles/coder)

# Load environment variables from ~/.whox/.env so API key checks work
...
from whox_constants import OPENROUTER_MODELS_URL
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.whox/.env contains provider auth/base URL settings."""
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `whox`.
    os.environ.setdefault("WHOX_INTERACTIVE", "1")
    ...
    print(color("│                 🩺 WHOX Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    # Check ~/.whox/.env (primary location for user config)
    env_path = WHOX_HOME / '.env'
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'whox setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'whox setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'whox setup' to create one")
                issues.append("Run 'whox setup' to create .env")
    
    # Check ~/.whox/config.yaml (primary) or project cli-config.yaml (fallback)
    ...
    if config_path.exists():
        ...
        if fallback_config.exists():
            ...
            example_config = PROJECT_ROOT / 'config' / 'examples' / 'cli-config.yaml.example'
            if not example_config.exists():
                example_config = PROJECT_ROOT / 'cli-config.yaml.example'
            if should_fix and example_config.exists():
                config_path.parent.mkdir(parents=True, exist_ok=True)
                shutil.copy2(str(example_config), str(config_path))
                check_ok(f"Created {_DHH}/config.yaml from template")
                fixed_count += 1
            elif should_fix:
    ...
    if config_path.exists():
        try:
            from whox_cli.config import check_config_version, migrate_config
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'whox setup' to migrate config")
                else:
                    issues.append("Run 'whox doctor --fix' or 'whox setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'whox doctor --fix'")
        except Exception:
            pass
    ...
    try:
        from whox_cli.auth import get_nous_auth_status, get_codex_auth_status
    ...
    if whox_home.exists():
        ...
        if should_fix:
            whox_home.mkdir(parents=True, exist_ok=True)
    ...
    for subdir_name in expected_subdirs:
        subdir_path = whox_home / subdir_name
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give WHOX a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# WHOX Agent Persona\n\n"
                "<!-- Edit this file to customize how WHOX communicates. -->\n\n"
                "You are WHOX, a helpful AI assistant.\n",
                encoding="utf-8",
            )
    ...
    memories_dir = whox_home / "memories"
    ...
    state_db_path = whox_home / "state.db"
    ...
    wal_path = whox_home / "state.db-wal"
    if wal_path.exists():
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'whox doctor --fix' to checkpoint")
    ...
    _apikey_providers = [
        ...
        # MiniMax APIs don't support /models endpoint — https://github.com/NousResearch/whox-agent/issues/811
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'whox setup' to configure missing API keys for full tool access")
    except Exception as e:
    ...
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: whox skills list)")
    ...
    try:
        ...
        if not _honcho_cfg_path.exists():
            check_warn("Honcho config not found", "run: whox memory setup")
        elif not hcfg.enabled:
            check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
        elif not (hcfg.api_key or hcfg.base_url):
            check_fail("Honcho API key or base URL not set", "run: whox memory setup")
            issues.append("No Honcho API key — run 'whox memory setup'")
        else:
    ...
    try:
        ...
        if mem0_key:
            ...
            check_warn("Mem0 not configured", "(set MEM0_API_KEY in .env or run whox memory setup)")
    except ImportError:
    ...
    # =========================================================================
    try:
        from whox_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
                ...
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "whox -p" in content:
                            _m = _re.search(r"whox -p (\S+)", content)
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'whox doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
