"""whox webhook — manage dynamic webhook subscriptions from the CLI.

Usage:
    whox webhook subscribe <name> [options]
    whox webhook list
    whox webhook remove <name>
    whox webhook test <name> [--payload '{"key": "value"}']

Subscriptions persist to ~/.whox/webhook_subscriptions.json and are
...
def _whox_home() -> Path:
    return Path(
        os.getenv("WHOX_HOME", str(Path.home() / ".whox"))
    ).expanduser()


def _subscriptions_path() -> Path:
    return _whox_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: whox gateway run
"""
...
def webhook_command(args):
    """Entry point for 'whox webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: whox webhook {subscribe|list|remove|test}")
        print("Run 'whox webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (whox gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: whox webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from whox webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (whox gateway run)")
