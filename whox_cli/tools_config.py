"""
Unified tool configuration for WHOX Agent.

`whox tools` and `whox setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.whox/config.yaml under
...
# They're still in _WHOX_CORE_TOOLS (available at runtime if enabled),
...
def _get_effective_configurable_toolsets():
    ...
    try:
        from whox_cli.plugins import discover_plugins, get_plugin_toolsets
...
PLATFORMS = {
    "cli":      {"label": "🖥️  CLI",       "default_toolset": "whox-cli"},
    "telegram": {"label": "📱 Telegram",   "default_toolset": "whox-telegram"},
    "discord":  {"label": "💬 Discord",    "default_toolset": "whox-discord"},
    "slack":    {"label": "💼 Slack",      "default_toolset": "whox-slack"},
    "whatsapp": {"label": "📱 WhatsApp",   "default_toolset": "whox-whatsapp"},
    "signal":   {"label": "📡 Signal",     "default_toolset": "whox-signal"},
    "homeassistant": {"label": "🏠 Home Assistant", "default_toolset": "whox-homeassistant"},
    "email":    {"label": "📧 Email",      "default_toolset": "whox-email"},
    "matrix":   {"label": "💬 Matrix",     "default_toolset": "whox-matrix"},
 "dingtalk": {"label": "💬 DingTalk", "default_toolset": "whox-dingtalk"},
    "feishu": {"label": "🪽 Feishu", "default_toolset": "whox-feishu"},
    "wecom": {"label": "💬 WeCom", "default_toolset": "whox-wecom"},
    "api_server": {"label": "🌐 API Server", "default_toolset": "whox-api-server"},
    "mattermost": {"label": "💬 Mattermost", "default_toolset": "whox-mattermost"},
    "webhook": {"label": "🔗 Webhook", "default_toolset": "whox-webhook"},
}
    ...
    if post_setup_key == "browserbase":
        ...
        if not node_modules.exists() and shutil.which("npm"):
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_whox_home()}/whox-agent && npm install")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in whox-agent directory)")
...
def _get_platform_tools(
    ...
    *,
    ...
    # "whox-cli" (which include all _WHOX_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # (e.g. "whox-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
    ...
    # A plugin toolset is "known" for a platform once `whox tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by whox tools — default enabled
                enabled_toolsets.add(pts)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (whox-cli, whox-telegram, etc.)
    ...
    if not env_vars:
        ...
        if managed_feature:
            ...
            if any(get_env_value(env_var) for env_var in override_envs):
                _print_warning(
                    "  Direct credentials are still configured and may take precedence until you remove them from ~/.whox/.env."
                )
        return
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `whox tools` and `whox setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚕ WHOX Tool Configuration", Colors.CYAN, Colors.BOLD))
    ...
    print()
    from whox_constants import display_whox_home
    print(color(f"  Tool configuration saved to {display_whox_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'whox' or gateway restart.", Colors.DIM))
    print()
