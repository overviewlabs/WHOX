...
1. **User plugins**   – ``~/.whox/plugins/<name>/``
2. **Project plugins** – ``./.whox/plugins/<name>/`` (opt-in via
   ``WHOX_ENABLE_PROJECT_PLUGINS``)
3. **Pip plugins**     – packages that expose the ``whox_agent.plugins``
   entry-point group.
...
ENTRY_POINTS_GROUP = "whox_agent.plugins"

_NS_PARENT = "whox_plugins"
...
class PluginManager:
    ...
    def discover_and_load(self) -> None:
        ...
        # 1. User plugins (~/.whox/plugins/)
        whox_home = os.environ.get("WHOX_HOME", os.path.expanduser("~/.whox"))
        user_dir = Path(whox_home) / "plugins"
        manifests.extend(self._scan_directory(user_dir, source="user"))

        # 2. Project plugins (./.whox/plugins/)
        if _env_enabled("WHOX_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".whox" / "plugins"
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``whox_plugins.<name>``."""
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``whox tools`` TUI so plugin-provided toolsets appear
