"""whox memory setup|status — configure memory provider plugins.
...
# ---------------------------------------------------------------------------
# Curses-based interactive picker (same pattern as whox tools)
# ---------------------------------------------------------------------------
...
def _install_dependencies(provider_name: str) -> None:
    ...
    if not uv_path:
        ...
        print(f"  Then re-run: whox memory setup")
        return
...
def cmd_setup(args) -> None:
    ...
    if not providers:
        ...
        print("  Install a plugin to ~/.whox/plugins/ and try again.\n")
        return
    ...
    env_path = Path(os.environ.get("WHOX_HOME", os.path.expanduser("~/.whox"))) / ".env"
    env_writes = {}
    ...
    whox_home = str(Path(os.environ.get("WHOX_HOME", os.path.expanduser("~/.whox"))))
    if provider_config and hasattr(provider, "save_config"):
        try:
            provider.save_config(provider_config, whox_home)
        except Exception as e:
...
def cmd_status(args) -> None:
    ...
    if provider_name:
        ...
        if found:
            ...
            print(f"  Install the '{provider_name}' memory plugin to ~/.whox/plugins/")
