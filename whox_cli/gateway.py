"""
Gateway subcommand for whox CLI.

Handles: whox gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
from whox_cli.config import get_env_value, get_whox_home, save_env_value, is_managed, managed_error
# display_whox_home is imported lazily at call sites to avoid ImportError
# when whox_constants is cached from a pre-update version during `whox update`.
from whox_cli.setup import (
...
def find_gateway_pids() -> list:
    ...
    pids = []
    patterns = [
        "whox_cli.main gateway",
        "whox_cli/main.py gateway",
        "whox gateway",
        "gateway/run.py",
    ]
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (WHOX_HOME-scoped).
...
_SERVICE_BASE = "whox-gateway"
SERVICE_DESCRIPTION = "WHOX Agent Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current WHOX_HOME.

    Returns ``""`` for the default ``~/.whox``, the profile name for
    ``~/.whox/profiles/<name>``, or a short hash for any other custom
    WHOX_HOME path.
    """
    import hashlib
    import re
    from pathlib import Path as _Path
    home = get_whox_home().resolve()
    default = (_Path.home() / ".whox").resolve()
    if home == default:
        return ""
    # Detect ~/.whox/profiles/<name> pattern → use the profile name
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary WHOX_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def get_service_name() -> str:
    """Derive a systemd service name scoped to this WHOX_HOME.

    Default ``~/.whox`` returns ``whox-gateway`` (backward compatible).
    Profile ``~/.whox/profiles/coder`` returns ``whox-gateway-coder``.
    Any other WHOX_HOME appends a short hash for uniqueness.
    """
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    whox gateway uninstall")
    print_info("    sudo whox gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:
            print_warning("  System service install requires sudo, so WHOX can't create it from this user session.")
            if run_as_user:
                print_info(f"  After setup, run: sudo whox gateway install --system --run-as-user {run_as_user}")
            else:
                print_info("  After setup, run: sudo whox gateway install --system --run-as-user <your-user>")
            print_info("  Then start it with: sudo whox gateway start --system")
            return scope, False
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.whox`` → ``ai.whox.gateway.plist`` (backward compatible).
    Profile ``~/.whox/profiles/coder`` → ``ai.whox.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.whox.gateway-{suffix}" if suffix else "ai.whox.gateway"
...
def get_whox_cli_path() -> str:
    """Get the path to the whox CLI."""
    ...
    return f"{get_python_path()} -m whox_cli.main"
...
def _whox_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current WHOX_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_whox_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.whox                    → /home/alice/.whox
      /root/.whox/profiles/coder     → /home/alice/.whox/profiles/coder
      /opt/custom-whox               → /opt/custom-whox  (kept as-is)
    """
    current_whox = get_whox_home().resolve()
    current_default = (Path.home() / ".whox").resolve()
    target_default = Path(target_home_dir) / ".whox"

    # Default ~/.whox → remap to target user's default
    if current_whox == current_default:
        return str(target_default)

    # Profile or subdir of ~/.whox → preserve the relative structure
    try:
        relative = current_whox.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.whox) — keep as-is
        return str(current_whox)
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        whox_home = _whox_home_for_target_user(home_dir)
...
User={username}
Group={group_name}
ExecStart={python_path} -m whox_cli.main gateway run --replace
...
Environment="WHOX_HOME={whox_home}"
Restart=on-failure
RestartSec=30
KillMode=mixed
KillSignal=SIGTERM
...
"""
    ...
    whox_home = str(get_whox_home().resolve())
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    print(f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current WHOX install")
    return True
...
def systemd_install(force: bool = False, system: bool = False, run_as_user: str | None = None):
    ...
    print()
    print("Next steps:")
    print(f"  {'sudo ' if system else ''}whox gateway start{scope_flag}              # Start the service")
    print(f"  {'sudo ' if system else ''}whox gateway status{scope_flag}             # Check status")
...
def systemd_status(deep: bool = False, system: bool = False):
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}whox gateway install{scope_flag}")
        return
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {'sudo ' if system else ''}whox gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}whox gateway start{scope_flag}")
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.whox.gateway-{suffix}" if suffix else "ai.whox.gateway"
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print("↻ Updated gateway launchd service definition to match the current WHOX install")
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  whox gateway status             # Check status")
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current WHOX install")
    else:
        print("⚠ Service definition is stale relative to the current WHOX install")
        print("  Run: whox gateway start")
    ...
    if deep:
        log_file = get_whox_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and WHOX will log in directly",
        ...
        "vars": [
            ...
            {"name": "MATRIX_USER_ID", "prompt": "User ID (@bot:server — required for password login)", "password": False,
             "help": "Full Matrix user ID, e.g. @whox:matrix.example.org"},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. whox) and copy the bot token",
        ...
        "vars": [
            ...
            {"name": "MATTERMOST_HOME_CHANNEL", "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)", "password": False,
             "help": "Channel ID where WHOX delivers cron results and notifications."},
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your WHOX agent",
        ...
        "vars": [
            {"name": "EMAIL_ADDRESS", "prompt": "Email address", "password": False,
             "help": "The email address WHOX will use (e.g., whox@gmail.com)."},
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_whox_home() / "whatsapp" / "session" / "creds.json"
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'whox pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info("  Approve with: whox pairing approve {platform} {code}")
                else:
                    print_info("  Skipped — configure later with 'whox gateway setup'")
            continue
...
def _setup_whatsapp():
    ...
    from whox_cli.main import cmd_whatsapp
    import argparse
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info("    signal-cli link -n \"WHOXAgent\"")
    ...
    if any_configured:
        ...
        if service_running:
            if prompt_yes_no("  Restart the gateway to pick up changes?", True):
                try:
                    if is_linux():
                        ...
                        print_info("Start manually: whox gateway")
                except subprocess.CalledProcessError as e:
            ...
            if is_linux() or is_macos():
                ...
                print_info("  Run in foreground: whox gateway")
    else:
        print()
        print_info("No platforms configured. Run 'whox gateway setup' when ready.")
...
def gateway_command(args):
    ...
    if subcmd == "install":
        ...
        if is_linux():
            ...
            print("Run manually: whox gateway run")
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: whox gateway start")
                sys.exit(1)
        ...
        if is_linux() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                print("  whox gateway install")
                print("  sudo whox gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  whox gateway          # Run in foreground")
                print("  whox gateway install  # Install as user service")
                print("  sudo whox gateway install --system  # Install as boot-time system service")
