"""
Profile management for multiple isolated WHOX instances.

Each profile is a fully independent WHOX_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.whox/profiles/<name>/`` by default.

The "default" profile is ``~/.whox`` itself — backward compatible,
zero migration needed.

Usage::

    whox profile create coder          # fresh profile + bundled skills
    whox profile create coder --clone  # also copy config, .env, SOUL.md
    whox profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    whox -p coder chat                 # or via flag
    whox profile use coder             # set as sticky default
    whox profile delete coder          # remove profile + alias + service
"""
...
# Directories/files to exclude when exporting the default (~/.whox) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "whox-agent",         # repo checkout (multi-GB)
...
_RESERVED_NAMES = frozenset({
    "whox", "default", "test", "tmp", "root", "sudo",
})

# WHOX subcommands that cannot be used as profile names/aliases
_WHOX_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Always ``~/.whox/profiles/`` — anchored to the user's home,
    NOT to the current WHOX_HOME (which may itself be a profile).
    This ensures ``coder profile list`` can see all profiles.
    """
    return Path.home() / ".whox" / "profiles"


def _get_default_whox_home() -> Path:
    """Return the default (pre-profile) WHOX_HOME path."""
    return Path.home() / ".whox"


def _get_active_profile_path() -> Path:
    """Return the path to the sticky active_profile file."""
    return _get_default_whox_home() / "active_profile"
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its WHOX_HOME directory."""
    if name == "default":
        return _get_default_whox_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, whox subcommands, existing binaries in PATH.
    """
    if name in _RESERVED_NAMES:
        return f"'{name}' is a reserved name"
    if name in _WHOX_SUBCOMMANDS:
        return f"'{name}' conflicts with a whox subcommand"
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_whox_home()
...
def create_profile(
    ...
    if name == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.whox)."
        )
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches WHOX_HOME at module level.
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "WHOX_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if name == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.whox).\n"
            "To remove everything, use: whox uninstall"
        )
    ...
    # Check for service
    from whox_cli.gateway import _profile_suffix, get_service_name
    wrapper_path = _get_wrapper_dir() / name
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set WHOX_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("WHOX_HOME")
    try:
        os.environ["WHOX_HOME"] = str(profile_dir)
        from whox_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["WHOX_HOME"] = old_home
        elif "WHOX_HOME" in os.environ:
            del os.environ["WHOX_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.whox/active_profile``. Use ``"default"`` to clear.
    """
    validate_profile_name(name)
    if name != "default" and not profile_exists(name):
        raise FileNotFoundError(
            f"Profile '{name}' does not exist. "
            f"Create it with: whox profile create {name}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from WHOX_HOME.

    Returns ``"default"`` if WHOX_HOME is not set or points to ``~/.whox``.
    Returns the profile name if WHOX_HOME points into ``~/.whox/profiles/<name>``.
    Returns ``"custom"`` if WHOX_HOME is set to an unrecognized path.
    """
    from whox_constants import get_whox_home
    whox_home = get_whox_home()
    resolved = whox_home.resolve()

    default_resolved = _get_default_whox_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if name == "default":
        # The default profile IS ~/.whox itself — its parent is ~/ and its
        # directory name is ".whox", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: whox profile import <archive> --name <name>"
        )

    # Archives exported from the default profile have "default/" as top-level
    # dir.  Importing as "default" would target ~/.whox itself — disallow
    # that and guide the user toward a named profile.
    if inferred_name == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.whox). "
            "Specify a different name: whox profile import <archive> --name <name>"
        )
...
def generate_bash_completion() -> str:
    """Generate a bash completion script for whox profile names."""
    return '''# WHOX Agent profile completion
# Add to ~/.bashrc: eval "$(whox completion bash)"

_whox_profiles() {
    local profiles_dir="$HOME/.whox/profiles"
...
_whox_completion() {
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_whox_profiles)" -- "$cur"))
        return
    fi
...
complete -F _whox_completion whox
'''


def generate_zsh_completion() -> str:
    """Generate a zsh completion script for whox profile names."""
    return '''#compdef whox
# WHOX Agent profile completion
# Add to ~/.zshrc: eval "$(whox completion zsh)"

_whox() {
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.whox/profiles" ]]; then
        profiles+=("${(@f)$(ls $HOME/.whox/profiles 2>/dev/null)}")
    fi
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a WHOX_HOME path string.

    Called early in the CLI entry point, before any whox modules
    are imported, to set the WHOX_HOME environment variable.
    """
    ...
    if profile_name != "default" and not profile_dir.is_dir():
        raise FileNotFoundError(
            ...
            f"Create it with: whox profile create {profile_name}"
        )
