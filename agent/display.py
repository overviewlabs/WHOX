...
def build_tool_preview(tool_name: str, args: dict, max_len: int | None = None) -> str | None:
    ...
    *max_len* controls truncation.  ``None`` (default) defers to the global
    ...
    primary_args = {
        ...
        "browser_click": "ref", "browser_type": "text", "browser_download": "url",
...
def _render_inline_unified_diff(diff: str) -> list[str]:
    """Render unified diff lines in WHOX' inline transcript style."""
    rendered: list[str] = []
    from_file = None
    to_file = None
...
class KawaiiSpinner:
    ...
    def _animate(self):
        ...
        while self.running:
            if os.getenv("WHOX_SPINNER_PAUSE"):
                time.sleep(0.1)
                continue
...
def get_cute_tool_message(
    ...
    if tool_name == "browser_download":
        target = args.get("url", "")
        domain = target.replace("https://", "").replace("http://", "").split("/")[0] if target else "file"
        return _wrap(f"┊ ⬇️  download  {_trunc(domain, 35)}  {dur}")
