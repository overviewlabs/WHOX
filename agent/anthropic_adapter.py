"""Anthropic Messages API adapter for WHOX Agent.

Translates between WHOX's internal OpenAI-style message format and
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    WHOX historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by WHOX)
    ...
    # 1. WHOX-managed OAuth/setup token env var
    ...
    # This remains as a compatibility fallback for pre-migration WHOX configs.
    api_key = os.getenv("ANTHROPIC_API_KEY", "").strip()
    if api_key:
        return api_key
...
# ── WHOX-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.whox/.anthropic_oauth.json (our own file).

_OAUTH_CLIENT_ID = "9d1c250a-e61b-44d9-88ed-5944d1962f5e"
_OAUTH_TOKEN_URL = "https://console.anthropic.com/v1/oauth/token"
_OAUTH_REDIRECT_URI = "https://console.anthropic.com/oauth/code/callback"
_OAUTH_SCOPES = "org:create_api_key user:profile user:inference"
_WHOX_OAUTH_FILE = get_whox_home() / ".anthropic_oauth.json"
...
def run_whox_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run WHOX-native OAuth PKCE flow and return credential state."""
    import time
    import webbrowser
    ...
    print()
    print("Authorize WHOX with your Claude Pro/Max subscription.")
    print()
...
def run_whox_oauth_login() -> Optional[str]:
    """Run WHOX-native OAuth PKCE flow for Claude Pro/Max subscription.

    Opens a browser to claude.ai for authorization, prompts for the code,
    exchanges it for tokens, and stores them in ~/.whox/.anthropic_oauth.json.

    Returns the access token on success, None on failure.
    """
    result = run_whox_oauth_login_pure()
    if not result:
        return None
    ...
    _save_whox_oauth_credentials(access_token, refresh_token, expires_at_ms)
...
def _save_whox_oauth_credentials(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Save OAuth credentials to ~/.whox/.anthropic_oauth.json."""
    data = {
        "accessToken": access_token,
        "refreshToken": refresh_token,
        "expiresAt": expires_at_ms,
    ...
    try:
        _WHOX_OAUTH_FILE.parent.mkdir(parents=True, exist_ok=True)
        _WHOX_OAUTH_FILE.write_text(json.dumps(data, indent=2), encoding="utf-8")
        _WHOX_OAUTH_FILE.chmod(0o600)
    except (OSError, IOError) as e:
        logger.debug("Failed to save WHOX OAuth credentials: %s", e)


def read_whox_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read WHOX-managed OAuth credentials from ~/.whox/.anthropic_oauth.json."""
    if _WHOX_OAUTH_FILE.exists():
        try:
            data = json.loads(_WHOX_OAUTH_FILE.read_text(encoding="utf-8"))
            if data.get("accessToken"):
                return data
        except (json.JSONDecodeError, OSError, IOError) as e:
            logger.debug("Failed to read WHOX OAuth credentials: %s", e)
    return None


def refresh_whox_oauth_token() -> Optional[str]:
    """Refresh the WHOX-managed OAuth token using the stored refresh token.

    Returns the new access token, or None if refresh fails.
    """
    creds = read_whox_oauth_credentials()
    ...
    try:
        ...
        logger.debug("Successfully refreshed WHOX OAuth token")
        return refreshed["access_token"]
    except Exception as e:
        logger.debug("Failed to refresh WHOX OAuth token: %s", e)
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("WHOX Agent", "Claude Code")
                text = text.replace("WHOX agent", "Claude Code")
                text = text.replace("whox-agent", "claude-code")
