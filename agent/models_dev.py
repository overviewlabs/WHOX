...
Data is cached in memory (1hr TTL) and on disk (~/.whox/models_dev_cache.json)
...
# Provider ID mapping: WHOX provider names → models.dev provider IDs
...
def _get_cache_path() -> Path:
    ...
    env_val = os.environ.get("WHOX_HOME", "")
    whox_home = Path(env_val) if env_val else Path.home() / ".whox"
    return whox_home / "models_dev_cache.json"
