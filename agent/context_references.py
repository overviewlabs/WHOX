...
_SENSITIVE_WHOX_DIRS = (Path("skills") / ".hub",)
...
def _ensure_reference_path_allowed(path: Path) -> None:
    ...
    whox_home = Path(
        os.getenv("WHOX_HOME", str(home / ".whox"))
    ).expanduser().resolve()

    blocked_exact = {home / rel for rel in _SENSITIVE_HOME_FILES}
    blocked_exact.add(whox_home / ".env")
    blocked_dirs = [home / rel for rel in _SENSITIVE_HOME_DIRS]
    blocked_dirs.extend(whox_home / rel for rel in _SENSITIVE_WHOX_DIRS)
    ...
    for blocked_dir in blocked_dirs:
        try:
            ...
            continue
        raise ValueError("path is a sensitive credential or internal WHOX path and cannot be attached")
