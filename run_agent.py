...
_loaded_env_paths = load_whox_dotenv(whox_home=_whox_home, project_env=_project_env)
...
from agent.prompt_builder import build_skills_system_prompt, build_context_files_prompt, load_soul_md, TOOL_USE_ENFORCEMENT_GUIDANCE, TOOL_USE_ENFORCEMENT_MODELS, DEVELOPER_ROLE_MODELS, GOOGLE_MODEL_OPERATIONAL_GUIDANCE, CRON_SCHEDULING_GUIDANCE
...
_LLM_RPM_LOCK = threading.Lock()
_LLM_RPM_WINDOWS: Dict[str, List[float]] = {}
_LLM_LAST_REQUEST_TS: Dict[str, float] = {}
_LLM_RATE_LIMIT_COOLDOWN_UNTIL: Dict[str, float] = {}


class _SafeWriter:
    """Transparent stdio wrapper that catches OSError/ValueError from broken pipes.

    When whox-agent runs as a systemd service, Docker container, or headless
...
class AIAgent:
    ...
    def __init__(
        ...
        self._model_max_rpm = self._resolve_model_rpm_limit()
        self._model_min_request_interval = self._resolve_model_min_request_interval()
        self._model_max_input_tokens = self._resolve_max_input_tokens_per_request()
        self._model_rate_limit_cooldown_seconds = self._resolve_rate_limit_cooldown_seconds()
        self._api_max_retries = self._resolve_api_max_retries()
        self._rate_limit_max_retries = self._resolve_rate_limit_max_retries()
        self._model_rpm_key = f"{self.provider or 'auto'}::{(self.model or '').strip().lower()}"
        ...
        # WHOX is a tool-using client by default.
        ...
        # Persistent error log -- always writes WARNING+ to ~/.whox/logs/errors.log
            ...
            if api_key and base_url:
                ...
                if "openrouter" in effective_base.lower():
                    client_kwargs["default_headers"] = {
                        "HTTP-Referer": "https://whox-agent.nousresearch.com",
                        "X-OpenRouter-Title": "WHOX Agent",
                        "X-OpenRouter-Categories": "productivity,cli-agent",
                    }
                elif "api.githubcopilot.com" in effective_base.lower():
                    from whox_cli.models import copilot_default_headers
                ...
                if _routed_client is not None:
                    ...
                    if _explicit and _explicit not in ("auto", "openrouter", "custom"):
                        raise RuntimeError(
                            ...
                            f"variable, or switch to a different provider with `whox model`."
                        )
        ...
        # Session logs go into ~/.whox/sessions/ alongside gateway sessions
        whox_home = get_whox_home()
        self.logs_dir = whox_home / "sessions"
        ...
        if self._session_db:
            try:
                self._session_db.create_session(
                    ...
                    source=self.platform or os.environ.get("WHOX_SESSION_SOURCE", "cli"),
                    model=self.model,
                    model_config={
        ...
        try:
            from whox_cli.config import load_config as _load_agent_config
            ...
            try:
                ...
                if not _mem_provider_name:
                    try:
                        ...
                        if _hcfg.enabled and (_hcfg.api_key or _hcfg.base_url):
                            ...
                            try:
                                from whox_cli.config import load_config as _lc, save_config as _sc
                                _cfg = _lc()
                ...
                if _mem_provider_name:
                    ...
                    if self._memory_manager.providers:
                        from whox_constants import get_whox_home as _ghh
                        _init_kwargs = {
                            "session_id": self.session_id,
                        ...
                        try:
                            ...
                            _init_kwargs["agent_workspace"] = "whox"
                        except Exception:
                            pass
    ...
    def _dump_api_request_debug(
        ...
        *,
        ...
        try:
            ...
            if env_var_enabled("WHOX_DUMP_REQUEST_STDOUT"):
    ...
    def _build_system_prompt(self, system_message: str = None) -> str:
        ...
        if self.valid_tool_names:
            prompt_parts.append(CAPABILITY_ROUTING_GUIDANCE)
            _toolsets = sorted(
                {
                    ts
                    for ts in (
                        get_toolset_for_tool(name) for name in self.valid_tool_names
                    )
                    if ts
                }
            )
            _tools = ", ".join(sorted(self.valid_tool_names))
            if _toolsets:
                prompt_parts.append(
                    f"Runtime capability inventory: toolsets={', '.join(_toolsets)}; tools={_tools}"
                )
            else:
                prompt_parts.append(f"Runtime capability inventory: tools={_tools}")
        if "cronjob" in self.valid_tool_names:
            prompt_parts.append(CRON_SCHEDULING_GUIDANCE)
        ...
        if not self.skip_context_files:
            ...
            # mode).  The gateway process runs from the whox-agent install
        ...
        from whox_time import now as _whox_now
        now = _whox_now()
        ...
        prompt_parts.append(
            "Important: The 'Conversation started' timestamp is historical. "
            "Use current per-turn realtime context for the actual current date/time."
        )
    ...
    def _try_refresh_nous_client_credentials(self, *, force: bool = True) -> bool:
        ...
        try:
            from whox_cli.auth import resolve_nous_runtime_credentials

            creds = resolve_nous_runtime_credentials(
                min_key_ttl_seconds=max(60, int(os.getenv("WHOX_NOUS_MIN_KEY_TTL_SECONDS", "1800"))),
                timeout_seconds=float(os.getenv("WHOX_NOUS_TIMEOUT_SECONDS", "15")),
                force_mint=force,
            )
        except Exception as exc:
    ...
    def _resolve_model_rpm_limit(self) -> int:
        """Resolve per-agent LLM request limit (requests/minute). 0 disables."""
        model_name = (self.model or "").strip().lower()
        # Model-specific default: Qwen lanes are capped at 40 RPM unless explicitly overridden.
        if "qwen" in model_name:
            raw = os.getenv("WHOX_QWEN_MAX_RPM", "").strip() or os.getenv("WHOX_LLM_MAX_RPM", "").strip() or "40"
        else:
            raw = os.getenv("WHOX_LLM_MAX_RPM", "").strip()
        try:
    ...
    def _resolve_model_min_request_interval(self) -> float:
        """Resolve enforced minimum spacing between LLM requests (seconds)."""
        model_name = (self.model or "").strip().lower()
        if "qwen" in model_name:
            raw = (
                os.getenv("WHOX_QWEN_MIN_REQUEST_INTERVAL_SECONDS", "").strip()
                or os.getenv("WHOX_LLM_MIN_REQUEST_INTERVAL_SECONDS", "").strip()
                or "1.7"
            )
        else:
            raw = os.getenv("WHOX_LLM_MIN_REQUEST_INTERVAL_SECONDS", "").strip() or "0"
        try:
            value = float(raw)
        except Exception:
            value = 0.0
    ...
    def _resolve_rate_limit_cooldown_seconds(self) -> float:
        """Resolve post-429 cooldown window shared by provider+model key."""
        model_name = (self.model or "").strip().lower()
        if "qwen" in model_name:
            raw = (
                os.getenv("WHOX_QWEN_RATE_LIMIT_COOLDOWN_SECONDS", "").strip()
                or os.getenv("WHOX_RATE_LIMIT_COOLDOWN_SECONDS", "").strip()
                or "75"
            )
        else:
            raw = os.getenv("WHOX_RATE_LIMIT_COOLDOWN_SECONDS", "").strip() or "30"
        try:
            value = float(raw)
        except Exception:
            value = 30.0
    ...
    def _resolve_max_input_tokens_per_request(self) -> int:
        """Resolve soft cap for input tokens before proactive compression."""
        model_name = (self.model or "").strip().lower()
        if "qwen" in model_name:
            raw = (
                os.getenv("WHOX_QWEN_MAX_INPUT_TOKENS", "").strip()
                or os.getenv("WHOX_MAX_INPUT_TOKENS", "").strip()
                or "90000"
            )
        else:
            raw = os.getenv("WHOX_MAX_INPUT_TOKENS", "").strip() or "0"
        try:
            value = int(raw)
        except Exception:
            value = 0
    ...
    def _resolve_api_max_retries(self) -> int:
        """Resolve generic API retries for transient failures."""
        raw = os.getenv("WHOX_API_MAX_RETRIES", "").strip() or "3"
        try:
            value = int(raw)
        except Exception:
            value = 3
        # 0 or negative means "effectively unlimited" retries.
        if value <= 0:
            return 1_000_000
        return max(1, value)

    def _resolve_rate_limit_max_retries(self) -> int:
        """Resolve retry budget specifically for 429/rate-limit failures."""
        raw = os.getenv("WHOX_RATE_LIMIT_MAX_RETRIES", "").strip() or "8"
        try:
            value = int(raw)
        except Exception:
            value = 8
    ...
    def _record_model_rate_limit_cooldown(self, retry_after_seconds: Optional[float] = None) -> float:
        """Set per-model cooldown after a 429 and return remaining seconds."""
        key = getattr(self, "_model_rpm_key", "") or (self.model or "").lower() or "default"
        now = time.monotonic()
        fallback_cooldown = getattr(self, "_model_rate_limit_cooldown_seconds", 0.0) or 0.0
        cooldown = retry_after_seconds if (retry_after_seconds and retry_after_seconds > 0) else fallback_cooldown
        if cooldown <= 0:
            return 0.0
        with _LLM_RPM_LOCK:
            prev_until = _LLM_RATE_LIMIT_COOLDOWN_UNTIL.get(key, 0.0)
            new_until = max(prev_until, now + cooldown)
            _LLM_RATE_LIMIT_COOLDOWN_UNTIL[key] = new_until
            return max(0.0, new_until - now)

    def _apply_model_rpm_limit(self) -> None:
        """Global process-wide limiter keyed by provider+model.

        Enforces both:
        1) Sliding-window max RPM
        2) Minimum inter-request spacing
        """
        max_rpm = getattr(self, "_model_max_rpm", 0) or 0
        min_interval = getattr(self, "_model_min_request_interval", None)
        if min_interval is None:
            min_interval = self._resolve_model_min_request_interval()
            self._model_min_request_interval = min_interval

        if max_rpm <= 0 and min_interval <= 0:
            return
        key = getattr(self, "_model_rpm_key", "") or (self.model or "").lower() or "default"
        while True:
            with _LLM_RPM_LOCK:
                now = time.monotonic()
                window = _LLM_RPM_WINDOWS.setdefault(key, [])
                cutoff = now - 60.0
                ...
                if max_rpm > 0 and len(window) >= max_rpm:
                    rpm_wait = max(0.05, 60.0 - (now - window[0]) + 0.01)

                if min_interval > 0:
                    last_ts = _LLM_LAST_REQUEST_TS.get(key, 0.0)
                    interval_wait = max(0.0, (last_ts + min_interval) - now)

                cooldown_until = _LLM_RATE_LIMIT_COOLDOWN_UNTIL.get(key, 0.0)
                if cooldown_until > now:
                    cooldown_wait = cooldown_until - now

                wait_for = max(rpm_wait, interval_wait, cooldown_wait)
                if wait_for <= 0:
                    window.append(now)
                    _LLM_LAST_REQUEST_TS[key] = now
                    return
            time.sleep(wait_for)
    ...
    def _interruptible_streaming_api_call(
        ...
        def _call_chat_completions():
            ...
            import httpx as _httpx
            self._apply_model_rpm_limit()
            _base_timeout = float(os.getenv("WHOX_API_TIMEOUT", 1800.0))
            _stream_read_timeout_raw = float(os.getenv("WHOX_STREAM_READ_TIMEOUT", 60.0))
            # 0 or negative means "indefinite" read timeout for streaming.
            _stream_read_timeout = None if _stream_read_timeout_raw <= 0 else _stream_read_timeout_raw
            stream_kwargs = {
                **api_kwargs,
                "stream": True,
        ...
        def _call():
            ...
            _max_stream_retries = int(os.getenv("WHOX_STREAM_RETRIES", 2))
            _infinite_stream_retries = _max_stream_retries <= 0
            _stream_attempt = 0
                ...
                while True:
                    try:
                        ...
                        if _is_timeout or _is_conn_err or _is_sse_conn_err:
                            ...
                            if _infinite_stream_retries or _stream_attempt < _max_stream_retries:
                                _attempt_total = "∞" if _infinite_stream_retries else str(_max_stream_retries + 1)
                                _hide_conn_status = os.getenv("WHOX_HIDE_CONNECTION_RETRY_STATUS", "1").strip().lower() in {"1", "true", "yes", "on"}
                                logger.info(
                                ...
                                if not _hide_conn_status:
                                    self._emit_status(
                                        f"⚠️ Connection to provider dropped "
                                        f"({type(e).__name__}). Reconnecting… "
                                        f"(attempt {_stream_attempt + 2}/{_attempt_total})"
                                    )
        ...
        _stream_stale_timeout_base = float(os.getenv("WHOX_STREAM_STALE_TIMEOUT", 180.0))
    ...
    def _build_api_kwargs(self, api_messages: list) -> dict:
        ...
        api_kwargs = {
            ...
            "timeout": float(os.getenv("WHOX_API_TIMEOUT", 1800.0)),
        }
        if self.tools:
        ...
        if _is_nous:
            extra_body["tags"] = ["product=whox-agent"]
    ...
    def _supports_reasoning_extra_body(self) -> bool:
        ...
        if "models.github.ai" in self._base_url_lower or "api.githubcopilot.com" in self._base_url_lower:
            try:
                from whox_cli.models import github_model_reasoning_efforts
    ...
    def _handle_max_iterations(self, messages: list, api_call_count: int) -> str:
        ...
        try:
            ...
            if _is_nous:
                summary_extra_body["tags"] = ["product=whox-agent"]
    ...
    def run_conversation(
        ...
        # Optional lifecycle status. Disabled by default for cleaner chat UX.
        if os.getenv("WHOX_SHOW_START_STATUS", "0").strip().lower() in {"1", "true", "yes", "on"}:
            self._emit_status("🚀 Starting work now. Initializing tools and context...")
        ...
        if self._cached_system_prompt is None:
            ...
            if stored_prompt:
                ...
                try:
                    from whox_cli.plugins import invoke_hook as _invoke_hook
                    _invoke_hook(
                        "on_session_start",
                        session_id=self.session_id,
                        model=self.model,
        ...
        # are reused.  The system prompt is WHOX's territory; plugins
        ...
        while api_call_count < self.max_iterations and self.iteration_budget.remaining > 0:
            ...
            for idx, msg in enumerate(messages):
                ...
                if idx == current_turn_user_idx and msg.get("role") == "user":
                    _injections = []
                    # Always inject live local time context at API-call time so
                    # long sessions remain current-time aware without relying on
                    # stale session-start timestamps.
                    try:
                        from whox_time import now as _whox_now
                        _rt_now = _whox_now()
                        _tz_abbr = _rt_now.tzname() or "Eastern Time"
                        _rt_line = (
                            f"[REALTIME CONTEXT: Current local date/time is "
                            f"{_rt_now.strftime('%A, %B %d, %Y %I:%M:%S %p')} "
                            f"{_tz_abbr}.]"
                        )
                        _injections.append(_rt_line)
                    except Exception:
                        pass
            ...
            # cache prefix.  The system prompt is reserved for WHOX internals.
            if effective_system:
                api_messages = [{"role": "system", "content": effective_system}] + api_messages
            ...
            # Proactive guardrail: shrink oversized requests before contacting
            # provider to avoid token-throughput rate-limit spikes (HTTP 429).
            max_input_tokens = int(getattr(self, "_model_max_input_tokens", 0) or 0)
            proactive_max_compression_attempts = 3
            if max_input_tokens > 0 and approx_tokens > max_input_tokens:
                compression_attempts += 1
                if compression_attempts > proactive_max_compression_attempts:
                    self._vprint(
                        f"{self.log_prefix}⚠️  Request remains oversized after proactive compression "
                        f"({approx_tokens:,} > {max_input_tokens:,}). Proceeding anyway.",
                        force=True,
                    )
                else:
                    self._emit_status(
                        f"🗜️ Request is large (~{approx_tokens:,} tokens), compressing before API call..."
                    )
            ...
            retry_count = 0
            max_retries = max(1, int(getattr(self, "_api_max_retries", 3) or 3))
            ...
            while retry_count < max_retries:
                try:
                    ...
                    if env_var_enabled("WHOX_DUMP_REQUESTS"):
                    ...
                    if (
                        ...
                        _dhh = _dhh_fn()
                        print(f"{self.log_prefix}     • Check ANTHROPIC_TOKEN in {_dhh}/.env for WHOX-managed OAuth/setup tokens")
                        print(f"{self.log_prefix}     • Check ANTHROPIC_API_KEY in {_dhh}/.env for API keys or legacy token values")
                        print(f"{self.log_prefix}     • For API keys: verify at https://console.anthropic.com/settings/keys")
                        print(f"{self.log_prefix}     • For Claude Code: run 'claude /login' to refresh, then retry")
                        print(f"{self.log_prefix}     • Clear stale keys: whox config set ANTHROPIC_TOKEN \"\"")
                        print(f"{self.log_prefix}     • Legacy cleanup: whox config set ANTHROPIC_API_KEY \"\"")
                    ...
                    if is_rate_limited:
                        max_retries = max(max_retries, int(getattr(self, "_rate_limit_max_retries", max_retries) or max_retries))
                    ...
                    if is_client_error:
                        ...
                        if status_code in (401, 403) or "unauthorized" in error_msg or "forbidden" in error_msg or "permission" in error_msg:
                            ...
                            self._vprint(f"{self.log_prefix}      • Is the key valid? Run: whox setup", force=True)
                    ...
                    if is_rate_limited:
                        ...
                        if _resp_headers and hasattr(_resp_headers, "get"):
                            ...
                            if _ra_raw:
                                try:
                                    _retry_after = min(int(_ra_raw), 600)  # Cap at 10 minutes
                                except (TypeError, ValueError):
                                    pass
                    cooldown_remaining = self._record_model_rate_limit_cooldown(_retry_after if is_rate_limited else None)
                    base_wait = _retry_after if _retry_after else min(2 ** retry_count, 60)
                    wait_time = max(base_wait, int(cooldown_remaining) if cooldown_remaining > 0 else 0)
                    _hide_rl_status = os.getenv("WHOX_HIDE_RATE_LIMIT_STATUS", "1").strip().lower() in {"1", "true", "yes", "on"}
                    if is_rate_limited:
                        if not _hide_rl_status:
