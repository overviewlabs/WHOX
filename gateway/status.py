...
The PID file lives at ``{WHOX_HOME}/gateway.pid``.  WHOX_HOME defaults to
``~/.whox`` but can be overridden via the environment variable.  This means
separate WHOX_HOME directories naturally get separate PID files — a property
...
_GATEWAY_KIND = "whox-gateway"
_RUNTIME_STATUS_FILE = "gateway_state.json"
_LOCKS_DIRNAME = "gateway-locks"


def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting WHOX_HOME."""
    home = get_whox_home()
...
def _get_lock_dir() -> Path:
    ...
    override = os.getenv("WHOX_GATEWAY_LOCK_DIR")
    if override:
        return Path(override)
    state_home = Path(os.getenv("XDG_STATE_HOME", Path.home() / ".local" / "state"))
    return state_home / "whox" / _LOCKS_DIRNAME
...
def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the WHOX gateway."""
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different WHOX_HOME dirs).
    """
