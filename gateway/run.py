...
# Resolve WHOX home directory (respects WHOX_HOME override)
from whox_constants import get_whox_home
from utils import atomic_yaml_write
_whox_home = get_whox_home()

# Load environment variables from ~/.whox/.env first.
# User-managed env files should override stale shell exports on restart.
from dotenv import load_dotenv  # backward-compat for tests that monkeypatch this symbol
from whox_cli.env_loader import load_whox_dotenv
_env_path = _whox_home / '.env'
load_whox_dotenv(whox_home=_whox_home, project_env=Path(__file__).resolve().parents[1] / '.env')

# Bridge config.yaml values into the environment so os.getenv() picks them up.
# config.yaml is authoritative for terminal settings — overrides .env.
_config_path = _whox_home / 'config.yaml'
if _config_path.exists():
    try:
        ...
        from whox_cli.config import _expand_env_vars
        ...
        if _agent_cfg and isinstance(_agent_cfg, dict):
            if "max_turns" in _agent_cfg:
                os.environ["WHOX_MAX_ITERATIONS"] = str(_agent_cfg["max_turns"])
        # Timezone: bridge config.yaml → WHOX_TIMEZONE env var.
        # WHOX_TIMEZONE from .env takes precedence (already in os.environ).
        _tz_cfg = _cfg.get("timezone", "")
        if _tz_cfg and isinstance(_tz_cfg, str) and "WHOX_TIMEZONE" not in os.environ:
            os.environ["WHOX_TIMEZONE"] = _tz_cfg.strip()
        # Security settings
        _security_cfg = _cfg.get("security", {})
        if isinstance(_security_cfg, dict):
            _redact = _security_cfg.get("redact_secrets")
            if _redact is not None:
                os.environ["WHOX_REDACT_SECRETS"] = str(_redact).lower()
    except Exception:
        pass  # Non-fatal; gateway can still run with .env values

# Gateway runs in quiet mode - suppress debug output and use cwd directly (no temp dirs)
os.environ["WHOX_QUIET"] = "1"

# Enable interactive exec approval for dangerous commands on messaging platforms
os.environ["WHOX_EXEC_ASK"] = "1"
...
def _expand_whatsapp_auth_aliases(identifier: str) -> set:
    ...
    session_dir = _whox_home / "whatsapp" / "session"
    resolved = set()
    queue = [normalized]
...
def _resolve_runtime_agent_kwargs() -> dict:
    ...
    from whox_cli.runtime_provider import (
    ...
    try:
        runtime = resolve_runtime_provider(
            requested=os.getenv("WHOX_INFERENCE_PROVIDER"),
        )
            ...
            for skill_md in skills_dir.rglob("SKILL.md"):
                ...
                if name == normalized and name in disabled:
                    return (
                        ...
                        f"Enable it with: `whox skills config`"
                    )

        # Check optional skills (shipped with repo but not installed)
        from whox_constants import get_whox_home, get_optional_skills_dir
            ...
            for skill_md in optional_dir.rglob("SKILL.md"):
                ...
                if name == normalized:
                    ...
                    return (
                        ...
                        f"Install it with: `whox skills install {install_path}`"
                    )
    except Exception:
        pass
    return None
...
def _load_gateway_config() -> dict:
    """Load and parse ~/.whox/config.yaml, returning {} on any error."""
    try:
        config_path = _whox_home / 'config.yaml'
        if config_path.exists():
            import yaml
            with open(config_path, 'r', encoding='utf-8') as f:
                return yaml.safe_load(f) or {}
    except Exception:
        logger.debug("Could not load gateway config from %s", _whox_home / 'config.yaml')
    return {}
...
def _resolve_whox_bin() -> Optional[list[str]]:
    """Resolve the WHOX update command as argv parts.

    Tries in order:
    1. ``shutil.which("whox")`` — standard PATH lookup
    2. ``sys.executable -m whox_cli.main`` — fallback when WHOX is running
       from a venv/module invocation and the ``whox`` shim is not on PATH

    Returns argv parts ready for quoting/joining, or ``None`` if neither works.
    """
    import shutil

    whox_bin = shutil.which("whox")
    if whox_bin:
        return [whox_bin]
    ...
    try:
        ...
        if importlib.util.find_spec("whox_cli") is not None:
            return [sys.executable, "-m", "whox_cli.main"]
    except Exception:
        pass
...
class GatewayRunner:
    ...
    def __init__(self, config: Optional[GatewayConfig] = None):
        ...
        try:
            from whox_state import SessionDB
        ...
        # Track background task metadata for status queries
        # Key: task_id, Value: dict(kind, status, prompt, started_at, completed_at, error, source)
        self._background_task_meta: Dict[str, Dict[str, Any]] = {}
        # Pending execution-mode chooser state for long/build requests.
        # Key: session_key, Value: {"prompt": str, "created_at": float}
        self._pending_execution_mode: Dict[str, Dict[str, Any]] = {}
        # Track active in-flight task prompt per session for interrupt-resume mode.
        self._active_task_prompt: Dict[str, str] = {}
        # Runtime health monitor state (tools + skills).
        self._runtime_health_lock = threading.Lock()
        self._runtime_health_last_run = 0.0
    ...
    def _has_setup_skill(self) -> bool:
        """Check if the whox-agent-setup skill is installed."""
        try:
            from tools.skill_manager_tool import _find_skill
            return _find_skill("whox-agent-setup") is not None
        except Exception:
            return False

    # -- Voice mode persistence ------------------------------------------

    _VOICE_MODE_PATH = _whox_home / "gateway_voice_mode.json"
    ...
    def _cleanup_pending_execution_mode(self) -> None:
        """Drop stale execution-mode prompts to avoid accidental late matches."""
        ttl = float(os.getenv("WHOX_EXECUTION_MODE_TTL_SECONDS", "900"))
        now = time.time()
        stale = [
            sk for sk, data in self._pending_execution_mode.items()
            if not isinstance(data, dict) or (now - float(data.get("created_at", 0))) > ttl
        ]
        for sk in stale:
            self._pending_execution_mode.pop(sk, None)

    # -- Runtime health monitoring ---------------------------------------

    def _runtime_health_check_sync(self, reason: str = "periodic") -> dict:
        """Validate that core WHOX tools/skills are importable and registered.

        This is best-effort hardening. It cannot guarantee perfect uptime, but
        it proactively repairs common drift (missing registrations, stale
        compatibility imports, unsynced skills) before a user hits failures.
        """
        report = {"ok": True, "issues": []}

        # Keep bundled skills synced to runtime home.
        try:
            from tools.skills_sync import sync_skills
            sync_skills(quiet=True)
        except Exception as e:
            report["ok"] = False
            report["issues"].append(f"skills_sync failed: {type(e).__name__}: {e}")

        import importlib

        # Critical imports that must remain healthy for normal gateway usage.
        critical_modules = (
            "model_tools",
            "whox_cli.tools_config",
            "tools.skills_tool",
            "tools.web_tools",
            "tools.browser_tool",
            # Legacy compatibility import expected by older skills/scripts.
        ...
        for module_name in critical_modules:
            try:
                ...
                report["issues"].append(
                    f"import {module_name} failed: {type(e).__name__}: {e}"
                )

        try:
            from model_tools import get_all_tool_names, _discover_tools
            ...
            available = set(get_all_tool_names())
            ...
            if missing:
                report["ok"] = False
                report["issues"].append(f"missing required tools: {', '.join(missing)}")
        except Exception as e:
            report["ok"] = False
            report["issues"].append(f"tool registry check failed: {type(e).__name__}: {e}")

        if not report["ok"]:
            logger.warning(
                "Runtime health check (%s) found issues: %s",
                reason,
                " | ".join(report["issues"]),
            )
        else:
            logger.debug("Runtime health check (%s) passed", reason)

        return report

    async def _ensure_runtime_health(self, force: bool = False, reason: str = "periodic") -> None:
        """Run throttled runtime health checks for tools/skills."""
        interval = max(5.0, float(os.getenv("WHOX_RUNTIME_HEALTH_INTERVAL_SECONDS", "120")))
        now = time.time()
        if not force and (now - self._runtime_health_last_run) < interval:
            return
        if not self._runtime_health_lock.acquire(blocking=False):
            return
        try:
            self._runtime_health_last_run = now
            loop = asyncio.get_running_loop()
            await loop.run_in_executor(None, self._runtime_health_check_sync, reason)
        except Exception as e:
            logger.debug("Runtime health execution failed (%s): %s", reason, e)
        finally:
            self._runtime_health_lock.release()

    async def _runtime_health_watcher(self) -> None:
        """Periodic runtime self-check loop for long-lived gateway processes."""
        startup_delay = max(
            5.0,
            float(os.getenv("WHOX_RUNTIME_HEALTH_START_DELAY_SECONDS", "20")),
        )
        await asyncio.sleep(startup_delay)
        while self._running:
            try:
                await self._ensure_runtime_health(force=True, reason="watcher")
            except Exception as e:
                logger.debug("Runtime health watcher error: %s", e)
            interval = max(
                15,
                int(float(os.getenv("WHOX_RUNTIME_HEALTH_INTERVAL_SECONDS", "120"))),
            )
    ...
    @staticmethod
    def _parse_execution_mode_reply(text: str) -> Optional[str]:
        """Parse user's mode choice from free-form text."""
        t = (text or "").strip().lower()
        if not t:
            return None

        background_patterns = (
            "background", "sub agent", "sub-agent", "delegate",
            "in the background", "async", "asynchronous", "keep chatting",
        )
        main_patterns = (
            "main agent", "main", "foreground", "do it now",
            "here", "not background", "no delegate",
        )

        if any(p in t for p in background_patterns):
            return "background"
        if any(p in t for p in main_patterns):
            return "main"
        return None

    @staticmethod
    def _should_offer_execution_mode_choice(text: str) -> bool:
        """Heuristic for requests that are likely long-running/build-heavy."""
        t = (text or "").strip().lower()
        if not t or t.startswith("/"):
            return False

        build_like = re.search(
            r"\b(build|create|make|develop|scaffold|generate|edit|refactor)\b.{0,60}\b(app|application|site|website|webapp|web app|landing page|full[- ]stack)\b",
            t,
            re.IGNORECASE,
        )
        long_like = re.search(
            r"\b(long[- ]running|multi[- ]step|large task|big task|batch|background task|project)\b",
            t,
            re.IGNORECASE,
        )
        explicit_future_work = re.search(
            r"\b(deploy|publish|production|from scratch|end[- ]to[- ]end)\b",
            t,
            re.IGNORECASE,
        )
        return bool(build_like or long_like or explicit_future_work)
    ...
    def _start_background_subagent(
        ...
        *,
        ...
        """Launch a detached background sub-agent and return a user-facing status."""
        selected_label, specialist_prompt = self._select_specialist_subagent(prompt)
        if worker_label in ("Background sub-agent", "Background task", "auto"):
        ...
        progress_note = (
            "I will post progress updates as it works."
            if send_progress
            else "I’ll post the final result when done."
        )
        return (
            f'🔄 {worker_label} started: "{preview}"\n'
            f"Task ID: {task_id}\n"
            f"{progress_note} You can keep chatting here."
        )

    @staticmethod
    def _select_specialist_subagent(prompt: str) -> tuple[str, str]:
        """Pick a specialist sub-agent label and inject role guidance."""
        text = (prompt or "").lower()
        label = "Scout (General Specialist)"
        role_brief = (
            "general execution and coordination across tools with clear status reporting"
        )

        code_patterns = (
            "build app", "build site", "website", "web app", "frontend", "backend",
            "api", "react", "next.js", "nextjs", "typescript", "python", "code",
            "refactor", "bug", "fix", "deploy app",
        )
        research_patterns = (
            "research", "compare", "best", "alternatives", "sources", "cite",
            "investigate", "deep dive", "market analysis", "latest news",
            "search", "find the best", "free api", "free apis",
        )
        ops_patterns = (
            "vps", "server", "docker", "nginx", "caddy", "ssl", "domain", "dns",
            "kubernetes", "cron", "systemd", "monitoring", "infrastructure",
        )
        data_patterns = (
            "sql", "database", "analytics", "metrics", "dashboard", "csv", "etl",
            "pipeline", "report", "aggregation", "lead scoring",
        )

        if any(p in text for p in research_patterns):
            label = "Sage (Deep Research Specialist)"
            role_brief = "deep research, source validation, and synthesis"
        elif any(p in text for p in code_patterns):
            label = "Forge (Code & Web Specialist)"
            role_brief = "software engineering and full-stack web/app delivery"
        elif any(p in text for p in ops_patterns):
            label = "Ops (Infrastructure Specialist)"
            role_brief = "DevOps, deployments, reliability, and server operations"
        elif any(p in text for p in data_patterns):
            label = "Pulse (Data Specialist)"
            role_brief = "data engineering, analytics, and structured data workflows"

        specialist_prompt = (
            f"[Specialist Sub-Agent Role: {label}]\n"
            f"You are a delegated specialist focused on {role_brief}. "
            "Work end-to-end, do not stop midway, and keep status updates concise and concrete.\n\n"
            f"User task:\n{prompt}"
        )
        return label, specialist_prompt
    ...
    def _load_prefill_messages() -> List[Dict[str, Any]]:
        ...
        Checks WHOX_PREFILL_MESSAGES_FILE env var first, then falls back to
        the prefill_messages_file key in ~/.whox/config.yaml.
        Relative paths are resolved from ~/.whox/.
        """
        import json as _json
        file_path = os.getenv("WHOX_PREFILL_MESSAGES_FILE", "")
        if not file_path:
            try:
                import yaml as _y
                cfg_path = _whox_home / "config.yaml"
                if cfg_path.exists():
    ...
    def _load_ephemeral_system_prompt() -> str:
        ...
        Checks WHOX_EPHEMERAL_SYSTEM_PROMPT env var first, then falls back to
        agent.system_prompt in ~/.whox/config.yaml.
        """
        prompt = os.getenv("WHOX_EPHEMERAL_SYSTEM_PROMPT", "")
        if prompt:
            return prompt
        try:
            import yaml as _y
    ...
    def _load_reasoning_config() -> dict | None:
        ...
        WHOX_REASONING_EFFORT as a fallback. Valid: "xhigh", "high",
        "medium", "low", "minimal", "none". Returns None to use default
        (medium).
        """
        from whox_constants import parse_reasoning_effort
        effort = ""
        try:
            import yaml as _y
            cfg_path = _whox_home / "config.yaml"
            ...
            pass
        if not effort:
            effort = os.getenv("WHOX_REASONING_EFFORT", "")
    ...
    def _load_background_notifications_mode() -> str:
        ...
        """
        mode = os.getenv("WHOX_BACKGROUND_NOTIFICATIONS", "")
        if not mode:
            try:
                import yaml as _y
                cfg_path = _whox_home / "config.yaml"
    ...
    async def start(self) -> bool:
        ...
        """
        logger.info("Starting WHOX Gateway...")
        logger.info("Session storage: %s", self.config.sessions_dir)
        try:
            from whox_cli.profiles import get_active_profile_name
        ...
        if not _any_allowlist and not _allow_all:
            logger.warning(
                ...
                "Set GATEWAY_ALLOW_ALL_USERS=true in ~/.whox/.env to allow open access, "
        ...
        # Startup preflight so broken tool/skill imports are repaired before
        # first user message.
        await self._ensure_runtime_health(force=True, reason="startup")
        ...
        if not notified and any(
            path.exists()
            for path in (
                _whox_home / ".update_pending.json",
                _whox_home / ".update_pending.claimed.json",
            )
        ):
        ...
        # Start runtime health monitor for tools/skills.
        self._runtime_health_task = asyncio.create_task(self._runtime_health_watcher())
        self._background_tasks.add(self._runtime_health_task)
        self._runtime_health_task.add_done_callback(self._background_tasks.discard)
    ...
    def _create_adapter(
        ...
        if platform == Platform.TELEGRAM:
            ...
            if not check_slack_requirements():
                logger.warning("Slack: slack-bolt not installed. Run: pip install 'whox-agent[slack]'")
                return None
    ...
    def _get_busy_input_mode(self) -> str:
        """Return how to handle new user messages while an agent is running.

        Modes:
        - interrupt: default legacy behavior
        - queue: keep current run alive; process follow-up after completion
        - interrupt_resume: interrupt to answer latest message, then
          auto-resume the paused task.
        """
        mode = "interrupt"
        try:
            cfg = _load_gateway_config() or {}
            mode = str((cfg.get("display", {}) or {}).get("busy_input_mode", "interrupt")).strip().lower()
        except Exception:
            mode = "interrupt"
    ...
    async def _handle_message(self, event: MessageEvent) -> Optional[str]:
                ...
                if code:
                    ...
                    if adapter:
                        await adapter.send(
                            ...
                            f"`whox pairing approve {platform_name} {code}`"
                        )
                else:
                    adapter = self.adapters.get(source.platform)
                    if adapter:
        ...
        _raw_stale_timeout = float(os.getenv("WHOX_AGENT_TIMEOUT", 600))
        ...
        if _quick_key in self._running_agents:
            ...
            from whox_cli.commands import resolve_command as _resolve_cmd_inner
            ...
            # Optional queue mode: do not interrupt active run for normal
            # follow-up messages. This keeps long tasks alive and processes
            # user messages after completion.
            _busy_mode = self._get_busy_input_mode()
            if _busy_mode == "queue":
                adapter = self.adapters.get(source.platform)
                if adapter:
                    # Preserve latest follow-up for next turn.
                    adapter._pending_messages[_quick_key] = event
                return "Queued for next turn while I finish the current task."

            # interrupt_resume mode: answer newest follow-up now, then
            # automatically continue the paused task.
            if _busy_mode == "interrupt_resume" and event.message_type == MessageType.TEXT and (event.text or "").strip():
                _paused_prompt = (self._active_task_prompt.get(_quick_key) or "").strip()
                _augmented = event.text
                if _paused_prompt:
                    _augmented = (
                        f"{event.text}\n\n"
                        "[System instruction: You were in the middle of a task before this interruption. "
                        "First address the user's newest message, then automatically resume and complete "
                        f"the paused task: {_paused_prompt}]"
                    )

                running_agent = self._running_agents.get(_quick_key)
                if running_agent and running_agent is not _AGENT_PENDING_SENTINEL:
                    logger.debug("INTERRUPT_RESUME for session %s", _quick_key[:20])
                    running_agent.interrupt(_augmented)

                adapter = self.adapters.get(source.platform)
                if adapter:
                    from gateway.platforms.base import MessageEvent as _ME, MessageType as _MT
                    adapter._pending_messages[_quick_key] = _ME(
                        text=_augmented,
                        message_type=_MT.TEXT,
                        source=event.source,
                        message_id=event.message_id,
        ...
        # in whox_cli/commands.py — no hardcoded set to maintain here.
        from whox_cli.commands import GATEWAY_KNOWN_COMMANDS, resolve_command as _resolve_cmd
        ...
        if command:
            try:
                from whox_cli.plugins import get_plugin_command_handler
        ...
        # Long-task execution mode chooser disabled:
        # WHOX should follow normal behavior and handle the user message
        # directly without prompting main/background mode.
        self._cleanup_pending_execution_mode()
        self._pending_execution_mode.pop(_quick_key, None)
    ...
    async def _handle_message_with_agent(self, event, source, _quick_key: str):
        ...
        await self._ensure_runtime_health(force=False, reason="pre-message")

        # Get or create session
        session_entry = self.session_store.get_or_create_session(source)
        session_key = session_entry.session_key
        if event.message_type == MessageType.TEXT and (event.text or "").strip():
            self._active_task_prompt[_quick_key] = event.text
        ...
        if history and len(history) >= 4:
            ...
            _hyg_api_key = None
            try:
                _hyg_cfg_path = _whox_home / "config.yaml"
            ...
            if not os.getenv(env_key):
                ...
                if adapter:
                    await adapter.send(
                        ...
                        f"A home channel is where WHOX delivers cron job results "
                    ...
                    if _stt_adapter:
                        try:
                            _stt_msg = (
                                ...
                                "(`pip install faster-whisper` in the WHOX venv) "
                                "and set `stt.enabled: true` in config.yaml, "
                                "then /restart the gateway."
                            )
                            # Point to setup skill if it's installed
                            if self._has_setup_skill():
                                _stt_msg += "\n\nFor full setup instructions, type: `/skill whox-agent-setup`"
        ...
        if event.media_urls and event.message_type == MessageType.DOCUMENT:
            for i, path in enumerate(event.media_urls):
                mtype = event.media_types[i] if i < len(event.media_types) else ""
                if mtype.startswith("image/") or mtype.startswith("audio/") or mtype.startswith("video/"):
                    continue
    ...
    async def _handle_profile_command(self, event: MessageEvent) -> str:
        ...
        from whox_constants import get_whox_home, display_whox_home
        from pathlib import Path

        home = get_whox_home()
        display = display_whox_home()

        # Detect profile name from WHOX_HOME path
        # Profile paths look like: ~/.whox/profiles/<name>
        profiles_parent = Path.home() / ".whox" / "profiles"
        try:
    ...
    async def _handle_help_command(self, event: MessageEvent) -> str:
        ...
        from whox_cli.commands import gateway_help_lines
        lines = [
            "📖 **WHOX Commands**\n",
            *gateway_help_lines(),
        ]
    ...
    async def _handle_provider_command(self, event: MessageEvent) -> str:
        ...
        if current_provider == "auto":
            try:
                from whox_cli.auth import resolve_provider as _resolve_provider
    ...
    async def _handle_personality_command(self, event: MessageEvent) -> str:
        ...
        if not personalities:
            return "No personalities configured in `~/.whox/config.yaml`"
        ...
        try:
            ...
            if "pynacl" in err_lower or "nacl" in err_lower or "davey" in err_lower:
                return (
                    ...
                    "Install or reinstall WHOX with the messaging extra, e.g. "
                    "`pip install whox-agent[messaging]`."
                )
    ...
    async def _send_voice_reply(self, event: MessageEvent, text: str) -> None:
        ...
        try:
            ...
            audio_path = os.path.join(
                tempfile.gettempdir(), "whox_voice",
    ...
    async def _handle_rollback_command(self, event: MessageEvent) -> str:
        ...
        try:
            import yaml as _y
            _cfg_path = _whox_home / "config.yaml"
            if _cfg_path.exists():
    ...
    async def _handle_background_command(self, event: MessageEvent) -> str:
        ...
        return self._start_background_subagent(
            ...
            worker_label="Background sub-agent",
            send_progress=True,
        )
    ...
    async def _run_background_task(
        ...
        worker_label: str = "Background task",
        ...
        try:
            ...
            if not runtime_kwargs.get("api_key"):
                ...
                if meta is not None:
                    ...
                    meta["error"] = "no provider credentials configured"
                await adapter.send(
                    source.chat_id,
                    f"❌ {worker_label} {task_id} failed: no provider credentials configured.",
            ...
            from whox_cli.tools_config import _get_platform_tools
            enabled_toolsets = sorted(_get_platform_tools(user_config, platform_key))

            pr = self._provider_routing
            max_iterations = int(os.getenv("WHOX_MAX_ITERATIONS", "90"))
            if max_iterations <= 0:
            ...
            if send_progress:
                ...
                progress_done = _threading.Event()

                def _natural_progress_text(raw: str) -> str:
                    text = str(raw or "").strip()
                    if not text:
                        return ""
                    # Keep progress updates conversational and clearly in-progress:
                    # strip common sentence punctuation markers fully.
                    text = re.sub(r"[:：\.。…]+", " ", text)
                ...
                async def _pump_progress() -> None:
                    ...
                    heartbeat_interval = float(
                        os.getenv("WHOX_BACKGROUND_PROGRESS_HEARTBEAT_SECONDS", "20")
                    )
                    while True:
                        try:
                            ...
                            if not progress_done.is_set():
                                ...
                                if (
                                    ...
                                    and (now - last_activity_at) >= heartbeat_interval
                                    and (now - last_sent_at) >= min_interval
                                ):
                                    try:
                                        await adapter.send(
                                            chat_id=source.chat_id,
                                            content=f"🧩 {worker_label} still working",
                                            metadata=_thread_metadata,
                                        )
                        ...
                        item_text = _natural_progress_text(item)
                        if not item_text or item_text == last_sent:
                            continue

                        now = time.monotonic()
                        if now - last_sent_at < min_interval:
                            await asyncio.sleep(min_interval - (now - last_sent_at))

                        try:
                            await adapter.send(
                                chat_id=source.chat_id,
                                content=f"🧩 {worker_label} {item_text}",
                                metadata=_thread_metadata,
                            )
                            last_sent = item_text
                            last_sent_at = time.monotonic()
                ...
                progress_pump_task = asyncio.create_task(_pump_progress())

            def run_sync():
                _last_tool = {"name": None}

                def _tool_progress_callback(tool_name: str, preview: str = None, args: dict = None):
                    if not progress_queue:
                        return
                    if tool_name == _last_tool["name"]:
                        return
                    _last_tool["name"] = tool_name
                    summary = preview or (", ".join((args or {}).keys()) if isinstance(args, dict) else "") or "working"
                    if len(summary) > 80:
                        summary = summary[:77] + "..."
                    progress_queue.put(f"using {tool_name} {summary}")

                def _status_callback(_event_type: str, message: str):
                    if progress_queue and message:
                ...
                agent = AIAgent(
                    ...
                    **turn_route["runtime"],
                    ...
                    tool_progress_callback=_tool_progress_callback if send_progress else None,
                    status_callback=_status_callback if send_progress else None,
                )
            ...
            if response:
                ...
                _display_prompt = (
                    (self._background_task_meta.get(task_id) or {}).get("prompt")
                    or prompt
                )
                preview = _display_prompt[:60] + ("..." if len(_display_prompt) > 60 else "")
                header = f'✅ {worker_label} complete\nPrompt: "{preview}"\n\n'
                ...
                await adapter.send(
                    chat_id=source.chat_id,
                    content=f'✅ {worker_label} complete\nPrompt: "{preview}"\n\n(No response generated)',
                    metadata=_thread_metadata,
                )
            ...
            try:
                await adapter.send(
                    chat_id=source.chat_id,
                    content=f"❌ {worker_label} {task_id} failed: {e}",
                    metadata=_thread_metadata,
                )
            except Exception:
                pass
            ...
            if progress_done:
                progress_done.set()
            if progress_pump_task:
                _flush_timeout_raw = float(
                    os.getenv("WHOX_BACKGROUND_PROGRESS_FLUSH_TIMEOUT", "0")
                )
                _flush_timeout = None if _flush_timeout_raw <= 0 else _flush_timeout_raw
                try:
                    if _flush_timeout is None:
                        await progress_pump_task
                    else:
                        await asyncio.wait_for(progress_pump_task, timeout=_flush_timeout)
                except Exception:
    ...
    async def _handle_yolo_command(self, event: MessageEvent) -> str:
        ...
        current = bool(os.environ.get("WHOX_YOLO_MODE"))
        if current:
            os.environ.pop("WHOX_YOLO_MODE", None)
            return "⚠️ YOLO mode **OFF** — dangerous commands will require approval."
        else:
            os.environ["WHOX_YOLO_MODE"] = "1"
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        """Handle /update command — update WHOX Agent to the latest version.

        Spawns ``whox update`` in a detached session (via ``setsid``) so it
        survives the gateway restart that ``whox update`` may trigger. Marker
        ...
        import subprocess
        from datetime import datetime
        from whox_cli.config import is_managed, format_managed_message

        if is_managed():
            return f"✗ {format_managed_message('update WHOX Agent')}"
        ...
        whox_cmd = _resolve_whox_bin()
        if not whox_cmd:
            return (
                "✗ Could not locate the `whox` command. "
                "WHOX is running, but the update command could not find the "
                "executable on PATH or via the current Python interpreter. "
                "Try running `whox update` manually in your terminal."
            )

        pending_path = _whox_home / ".update_pending.json"
        output_path = _whox_home / ".update_output.txt"
        exit_code_path = _whox_home / ".update_exit_code"
        pending = {
        ...
        # Spawn `whox update` detached so it survives gateway restart.
        # Use setsid for portable session detach (works under system services
        # where systemd-run --user fails due to missing D-Bus session).
        whox_cmd_str = " ".join(shlex.quote(part) for part in whox_cmd)
        update_cmd = (
            f"{whox_cmd_str} update > {shlex.quote(str(output_path))} 2>&1; "
        ...
        self._schedule_update_notification_watch()
        return "⚕ Starting WHOX update… I'll notify you when it's done."
    ...
    async def _watch_for_update_completion(
        ...
        """Wait for ``whox update`` to finish, then send its notification."""
        pending_path = _whox_home / ".update_pending.json"
        claimed_path = _whox_home / ".update_pending.claimed.json"
        exit_code_path = _whox_home / ".update_exit_code"
        loop = asyncio.get_running_loop()
            ...
            if adapter and chat_id:
                ...
                if output:
                    if len(output) > 3500:
                        output = "…" + output[-3500:]
                    if exit_code == 0:
                        msg = f"✅ WHOX update finished.\n\n```\n{output}\n```"
                    else:
                        msg = f"❌ WHOX update failed.\n\n```\n{output}\n```"
                else:
                    if exit_code == 0:
                        msg = "✅ WHOX update finished successfully."
                    else:
                        msg = "❌ WHOX update failed. Check the gateway logs or run `whox update` manually for details."
    ...
    def _set_session_env(self, context: SessionContext) -> None:
        ...
        os.environ["WHOX_SESSION_PLATFORM"] = context.source.platform.value
        os.environ["WHOX_SESSION_CHAT_ID"] = context.source.chat_id
        if context.source.chat_name:
            os.environ["WHOX_SESSION_CHAT_NAME"] = context.source.chat_name
        if context.source.thread_id:
            os.environ["WHOX_SESSION_THREAD_ID"] = str(context.source.thread_id)
    
    def _clear_session_env(self) -> None:
        """Clear session environment variables."""
        for var in ["WHOX_SESSION_PLATFORM", "WHOX_SESSION_CHAT_ID", "WHOX_SESSION_CHAT_NAME", "WHOX_SESSION_THREAD_ID"]:
    ...
    async def _enrich_message_with_transcription(
        ...
        if not getattr(self.config, "stt_enabled", True):
            ...
            if self._has_setup_skill():
                disabled_note += (
                    " You have a skill called whox-agent-setup that can help "
                    "users configure WHOX features including voice, tools, and more."
                )
            disabled_note += "]"
            if user_text:
            ...
            try:
                ...
                if result["success"]:
                    ...
                    if (
                        ...
                        if self._has_setup_skill():
                            _no_stt_note += (
                                " You have a skill called whox-agent-setup "
                                "that can help users configure WHOX features "
    ...
    async def _run_agent(
        ...
        progress_mode = (
            _raw_tp
            or os.getenv("WHOX_TOOL_PROGRESS_MODE")
            or "all"
        )
        ...
        # Telegram UX uses natural-language status bubbles instead of tool-level
        # formatting noise.
        ...
        async def send_progress_messages():
            ...
            _PROGRESS_EDIT_INTERVAL = float(
                os.getenv("WHOX_PROGRESS_EDIT_INTERVAL", "0.7")
        ...
        _status_progress_last_text = [None]
        _status_progress_last_sent_ts = [0.0]
        _status_progress_lock = asyncio.Lock()
        _status_progress_min_interval = float(
            os.getenv("WHOX_STATUS_PROGRESS_MIN_INTERVAL", "0.25")
        )
        def _natural_progress_text(raw: str) -> str:
            text = str(raw or "").strip()
            if not text:
        ...
        async def _set_ephemeral_progress(content: str) -> None:
            """Send status/progress as separate bubbles (no edits/deletes)."""
            if not _status_adapter or not content:
                return
            async with _status_progress_lock:
                content = _natural_progress_text(content)
                if not content:
                    return
                if content == _status_progress_last_text[0]:
                    return
                _now = time.monotonic()
                _remaining = _status_progress_min_interval - (_now - _status_progress_last_sent_ts[0])
                if _remaining > 0:
                    await asyncio.sleep(_remaining)
                _status_progress_last_text[0] = content
                await _status_adapter.send(
                    _status_chat_id,
                    content,
                    metadata=_status_thread_metadata,
                )
                _status_progress_last_sent_ts[0] = time.monotonic()

        async def _clear_ephemeral_progress() -> None:
            # No-op in "separate bubbles" mode.
            return
        ...
        def _status_callback_sync(event_type: str, message: str) -> None:
            ...
            try:
                asyncio.run_coroutine_threadsafe(
                    _set_ephemeral_progress(message),
                    _loop_for_step,
                )
            except Exception as _e:
        ...
        def run_sync():
            ...
            os.environ["WHOX_SESSION_KEY"] = session_key or ""
            ...
            def _bg_review_send(message: str) -> None:
                if not _status_adapter:
                    return
                clean = _natural_progress_text(message)
                if not clean:
                    return
                try:
                    asyncio.run_coroutine_threadsafe(
        ...
        async def monitor_for_interrupt():
            ...
            while True:
                ...
                if hasattr(adapter, 'has_pending_interrupt') and adapter.has_pending_interrupt(session_key):
                    agent = agent_holder[0]
                    if agent:
                        # IMPORTANT: do not dequeue here. Dequeuing in the
                        # monitor can drop user interrupts when the running
                        # agent does not stop immediately. Leave the pending
                        # message in adapter queue so post-run logic can
                        # reliably consume and process it.
                        pending_event = None
                        try:
                            _pending_map = getattr(adapter, "_pending_messages", None)
                            if isinstance(_pending_map, dict):
                                pending_event = _pending_map.get(session_key)
                        except Exception:
                            pending_event = None

                        pending_text = pending_event.text if pending_event else None
                        if pending_text and self._get_busy_input_mode() == "interrupt_resume":
                            pending_text = (
                                f"{pending_text}\n\n"
                                "[System instruction: First address the user's latest message, "
                                "then automatically resume and complete the interrupted task.]"
                            )
        ...
        # Configurable to keep users continuously updated.
        _NOTIFY_INTERVAL = float(
            os.getenv("WHOX_LONG_TASK_NOTIFY_INTERVAL", "0")
        )
        ...
        _notify_task = (
            asyncio.create_task(_notify_long_running())
            if _NOTIFY_INTERVAL > 0
            else None
        )
        ...
        try:
            ...
            _agent_timeout_raw = float(os.getenv("WHOX_AGENT_TIMEOUT", 600))
            ...
            try:
                ...
                response = {
                    "final_response": (
                        ...
                        "To increase the limit, set WHOX_AGENT_TIMEOUT in your .env "
            ...
            if pending:
                ...
                _busy_mode = self._get_busy_input_mode()
                if _busy_mode == "interrupt_resume":
                    # Enforce latest-message-first behavior even when the
                    # provider/tool run does not report `interrupted` quickly.
                    # This guarantees we prioritize the user's new message
                    # immediately, then continue the paused task.
                    _paused_prompt = (self._active_task_prompt.get(session_key) or "").strip()
                    if _paused_prompt and "[System instruction:" not in pending:
                        pending = (
                            f"{pending}\n\n"
                            "[System instruction: You were in the middle of a task before this interruption. "
                            "First address the user's newest message, then automatically resume and complete "
            ...
            # Default is no timeout so streaming can complete even for
            # long final flushes; set WHOX_STREAM_FINALIZE_TIMEOUT>0
            # to enforce a cap (seconds).
            if stream_task:
                _stream_finalize_timeout_raw = float(
                    os.getenv("WHOX_STREAM_FINALIZE_TIMEOUT", "0")
                )
                _stream_finalize_timeout = (
                    None
                    if _stream_finalize_timeout_raw <= 0
                    else _stream_finalize_timeout_raw
                )
                try:
                    if _stream_finalize_timeout is None:
                        await stream_task
                    else:
                        await asyncio.wait_for(stream_task, timeout=_stream_finalize_timeout)
                except asyncio.TimeoutError:
                    logger.warning(
                        "Stream finalize timeout after %.1fs; cancelling stream task",
                        _stream_finalize_timeout,
                    )
                    stream_task.cancel()
                    try:
            ...
            # Remove ephemeral progress bubble so only the final response remains.
            try:
                await _clear_ephemeral_progress()
            except Exception:
                pass
...
def _start_cron_ticker(stop_event: threading.Event, adapters=None, interval: int = 60):
    ...
    needing a separate `whox cron daemon` or system cron entry.
...
async def start_gateway(config: Optional[GatewayConfig] = None, replace: bool = False, verbosity: Optional[int] = 0) -> bool:
    ...
    # Prevent two gateways from running under the same WHOX_HOME.
    # The PID file is scoped to WHOX_HOME, so future multi-profile
    # setups (each profile using a distinct WHOX_HOME) will naturally
    ...
    if existing_pid is not None and existing_pid != os.getpid():
        if replace:
            ...
            whox_home = str(get_whox_home())
            logger.error(
                "Another gateway instance is already running (PID %d, WHOX_HOME=%s). "
                "Use 'whox gateway restart' to replace it, or 'whox gateway stop' first.",
                existing_pid, whox_home,
            )
            print(
                f"\n❌ Gateway already running (PID {existing_pid}).\n"
                f"   Use 'whox gateway restart' to replace it,\n"
                f"   or 'whox gateway stop' to kill it first.\n"
                f"   Or use 'whox gateway run --replace' to auto-replace.\n"
            )
            return False
...
def main():
    ...
    parser = argparse.ArgumentParser(description="WHOX Gateway - Multi-platform messaging")
