...
Checklist for integrating a new messaging platform into the WHOX gateway.
...
And add it to the `whox-gateway` composite:

```python
"whox-gateway": {
    "includes": [..., "whox-your-platform"]
}
```
...
## 12. Status Display (`whox_cli/status.py`)
...
## 13. Gateway Setup Wizard (`whox_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ whox_cli/ toolsets.py \
  --include="*.py" -l | sort -u
