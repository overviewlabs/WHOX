...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        _env_reply_mode = os.getenv("TELEGRAM_REPLY_TO_MODE", "").strip().lower()
        _cfg_reply_mode = str(getattr(config, "reply_to_mode", "off") or "off").strip().lower()
        _reply_mode = _env_reply_mode if _env_reply_mode in {"off", "first", "all"} else _cfg_reply_mode
        self._reply_to_mode: str = _reply_mode if _reply_mode in {"off", "first", "all"} else "off"
        # Buffer rapid/album photo updates so Telegram image bursts are handled
        # as a single MessageEvent instead of self-interrupting multiple turns.
        self._media_batch_delay_seconds = float(os.getenv("WHOX_TELEGRAM_MEDIA_BATCH_DELAY_SECONDS", "0.8"))
        ...
        self._text_batch_delay_seconds = float(os.getenv("WHOX_TELEGRAM_TEXT_BATCH_DELAY_SECONDS", "0.6"))
        self._typing_indicator_enabled = os.getenv("WHOX_TELEGRAM_TYPING_INDICATOR", "true").strip().lower() in ("1", "true", "yes", "on")
    ...
    async def _handle_polling_conflict(self, error: Exception) -> None:
        ...
        message = (
            ...
            "WHOX stopped Telegram polling after %d retries. "
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            if not acquired:
                ...
                message = (
                    "Another local WHOX gateway is already using this Telegram bot token"
            ...
            try:
                ...
                from whox_cli.commands import telegram_menu_commands
    ...
    async def delete_message(
        ...
        try:
            ...
            err_str = str(e).lower()
            # Deleted/already-gone/not found are effectively success for cleanup.
            if "message to delete not found" in err_str or "message can't be deleted" in err_str:
                return SendResult(success=True, message_id=message_id)
            logger.debug("[%s] Failed to delete Telegram message %s: %s", self.name, message_id, e)
            return SendResult(success=False, error=str(e))
    ...
    async def _handle_media_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        # Download document files to cache for agent processing
            ...
            try:
                ...
                # Resolve MIME type:
                # 1) Explicit allowlist entries (pdf/docx/txt/etc.)
                # 2) Telegram-declared image/* for "sent as file" images
                # 3) Standard MIME guess from filename extension
                resolved_mime = ""
                if ext in SUPPORTED_DOCUMENT_TYPES:
                    resolved_mime = SUPPORTED_DOCUMENT_TYPES[ext]
                elif doc.mime_type and doc.mime_type.lower().startswith("image/"):
                    resolved_mime = doc.mime_type.lower()
                elif doc.mime_type:
                    resolved_mime = doc.mime_type.split(";", 1)[0].strip().lower()
                elif ext:
                ...
                if resolved_mime.startswith("image/"):
                    image_ext = ext if ext in {".png", ".jpg", ".jpeg", ".webp", ".gif"} else ".jpg"
                    if resolved_mime.endswith("/png"):
                        image_ext = ".png"
                    elif resolved_mime.endswith("/webp"):
                        image_ext = ".webp"
                    elif resolved_mime.endswith("/gif"):
                        image_ext = ".gif"
                    cached_path = cache_image_from_bytes(raw_bytes, ext=image_ext)
                else:
                    fallback_name = f"document{ext or '.bin'}"
                    cached_path = cache_document_from_bytes(raw_bytes, original_filename or fallback_name)

                event.media_urls = [cached_path]
                event.media_types = [resolved_mime]
                logger.info("[Telegram] Cached user document at %s (%s)", cached_path, resolved_mime)

                # For small text-like files, inject content into event.text (capped at 100 KB)
                MAX_TEXT_INJECT_BYTES = 100 * 1024
                text_like_exts = {".md", ".txt", ".csv", ".json", ".xml", ".yaml", ".yml", ".log"}
                is_text_like = resolved_mime.startswith("text/") or ext in text_like_exts
                if is_text_like and len(raw_bytes) <= MAX_TEXT_INJECT_BYTES:
                    try:
