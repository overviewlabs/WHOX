...
# Uses get_whox_home() so each profile gets its own Matrix store.
from whox_constants import get_whox_dir as _get_whox_dir
_STORE_DIR = _get_whox_dir("platforms/matrix/store", "matrix/store")
...
_KEY_EXPORT_PASSPHRASE = "whox-matrix-e2ee-keys"
...
class MatrixAdapter(BasePlatformAdapter):
    ...
    async def _run_e2ee_maintenance(self) -> None:
        ...
        WHOX uses a custom sync loop instead of matrix-nio's sync_forever(),
    ...
    def _thread_state_path() -> Path:
        ...
        return get_whox_home() / "matrix_threads.json"
