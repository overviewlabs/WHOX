...
GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    ...
    "Load this skill in the local CLI to be prompted, or add the key to ~/.whox/.env manually."
)
...
# Default location: {WHOX_HOME}/cache/images/ (legacy: image_cache/)
IMAGE_CACHE_DIR = get_whox_dir("cache/images", "image_cache")
        ...
        for attempt in range(retries + 1):
            try:
                response = await client.get(
                    url,
                    headers={
                        "User-Agent": "Mozilla/5.0 (compatible; WHOXAgent/1.0)",
                        "Accept": "image/*,*/*;q=0.8",
                    },
                )
...
AUDIO_CACHE_DIR = get_whox_dir("cache/audio", "audio_cache")
...
DOCUMENT_CACHE_DIR = get_whox_dir("cache/documents", "document_cache")
...
class BasePlatformAdapter(ABC):
    ...
    async def delete_message(
        ...
        support deletion return success=False and callers ignore gracefully.
        """
        return SendResult(success=False, error="Not supported")
    ...
    def extract_media(content: str) -> Tuple[List[Tuple[str, bool]], str]:
        ...
        # Accept any local path (not only media extensions) so agents can send
        # arbitrary files as native attachments on platforms that support it.
    ...
    def extract_local_files(content: str) -> Tuple[List[str], str]:
        ...
        Matches absolute paths (/...) and tilde paths (~/) and validates each
        candidate with ``os.path.isfile()`` to avoid false positives from URLs
        ...
        # Require a filename extension for bare-path autodetection to avoid
        # partial matches on phrases like "/tmp/my file.png" -> "/tmp/my".
        path_re = re.compile(
            r'(?<![/:\w.])(?:~/|/)(?:[\w.\-]+/)*[\w.\-]+\.[\w.\-]+\b',
            re.IGNORECASE,
        )
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        if session_key in self._active_sessions:
            ...
            # Let the gateway handler decide busy-mode behavior (interrupt,
            # queue, interrupt_resume) and optionally return an immediate user
            # message. If it returns None and the handler is GatewayRunner's
            # _handle_message, treat as handled to avoid duplicate legacy queueing.
            try:
                _thread_meta = {"thread_id": event.source.thread_id} if event.source.thread_id else None
                response = await self._message_handler(event)
                if response:
                    ...
                    return
                _owner = getattr(self._message_handler, "__self__", None)
                if _owner and _owner.__class__.__name__ == "GatewayRunner":
                    return
            except Exception as e:
                logger.debug("[%s] Busy-session handler dispatch fell back to legacy queueing: %s", self.name, e)
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          WHOX_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          WHOX_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          WHOX_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        import random

        mode = os.getenv("WHOX_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        min_ms = int(os.getenv("WHOX_HUMAN_DELAY_MIN_MS", "800"))
        max_ms = int(os.getenv("WHOX_HUMAN_DELAY_MAX_MS", "2500"))
        if mode == "natural":
