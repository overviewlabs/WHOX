...
_DONE = object()
# Sentinel for an in-run stream boundary (e.g. assistant turn -> tool phase)
_RESET = object()
...
class GatewayStreamConsumer:
    ...
    def _with_sender_label(self, text: str) -> str:
        label = (self.cfg.sender_label or "").strip()
        if not label:
            return text
        prefix = f"{label}: "
    ...
    def on_delta(self, text: str) -> None:
        ...
        if text is None:
            # Boundary marker from agent loop: clear interim streamed content
            # before tool execution so only the final answer remains.
    ...
    async def run(self) -> None:
        ...
        try:
            while True:
                ...
                if got_reset:
                    # Drop any interim stream bubble from pre-tool assistant
                    # text so the eventual final response streams in its own
                    # clean message.
                    await self._reset_stream_message()
                    self._accumulated = ""
                ...
                if should_edit and self._accumulated:
                    ...
                    display_text = self._with_sender_label(self._accumulated)
                    if not got_done:
                ...
                if got_done:
                    ...
                    if self._accumulated and self._message_id:
                        await self._send_or_edit(self._with_sender_label(self._accumulated))
                    return
    ...
    async def _reset_stream_message(self) -> None:
        """Reset stream state at tool boundaries without deleting prior bubbles."""
        self._message_id = None
        self._last_sent_text = ""
        self._already_sent = False
