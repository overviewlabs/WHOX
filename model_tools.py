...
try:
    from whox_cli.plugins import discover_plugins
    discover_plugins()
except Exception as e:
...
def handle_function_call(
    ...
    try:
        # Self-heal registry drift before dispatch. This protects long-running
        # gateway processes where optional tool imports may have failed during
        # an earlier startup phase.
        if function_name not in registry.get_all_tool_names():
            try:
                _discover_tools()
            except Exception as e:
                logger.debug("Tool rediscovery before dispatch failed: %s", e)

        if function_name in _AGENT_LOOP_TOOLS:
            return json.dumps({"error": f"{function_name} must be handled by the agent loop"})

        try:
            from whox_cli.plugins import invoke_hook
        ...
        # If dispatch still reports unknown tool, attempt one more rediscovery
        # + retry before returning an error to the model/user.
        try:
            parsed = json.loads(result) if isinstance(result, str) else {}
            _err = str((parsed or {}).get("error", ""))
