...
Training tasks (3):
    1. Create ~/greeting.txt with "Hello from WHOX Agent"
...
TRAIN_TASKS = [
    {
        "prompt": "Create a file at ~/greeting.txt containing exactly the text: Hello from WHOX Agent",
        "verify_path": "~/greeting.txt",
        "expected_content": "Hello from WHOX Agent",
    },
    {
...
class TerminalTestEnv(WHOXAgentBaseEnv):
    ...
    def config_init(cls) -> Tuple[TerminalTestEnvConfig, List[APIServerConfig]]:
        ...
        Claude for inference. API keys loaded from ~/whox-agent/.env.
        """
