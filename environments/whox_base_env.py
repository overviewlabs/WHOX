"""
WHOXAgentBaseEnv -- Abstract Base Environment for WHOX-Agent + Atropos

Provides the Atropos integration plumbing that all whox-agent environments share:
- Two-mode operation (OpenAI server for Phase 1, VLLM ManagedServer for Phase 2)
- Per-group toolset/distribution resolution
- Agent loop orchestration via WHOXAgentLoop
...
# Ensure the whox-agent repo root is on sys.path so that imports like
...
# Load API keys from whox-agent/.env so all environments can access them
...
from environments.agent_loop import AgentResult, WHOXAgentLoop
from environments.tool_context import ToolContext

# Import whox-agent toolset infrastructure
...
class WHOXAgentEnvConfig(BaseEnvConfig):
    """
    Configuration for whox-agent Atropos environments.
    ...
    enabled_toolsets: Optional[List[str]] = Field(
        default=None,
        description="Explicit list of whox toolsets to enable (e.g., ['terminal', 'file', 'web']). "
    ...
    tool_call_parser: str = Field(
        ...
        "Options: whox, mistral, llama3_json, qwen, deepseek_v3, etc.",
    )
...
class WHOXAgentBaseEnv(BaseEnv):
    """
    Abstract base environment for whox-agent Atropos integration.
    ...
    name: Optional[str] = "whox-agent"
    env_config_cls = WHOXAgentEnvConfig

    def __init__(
        self,
        config: WHOXAgentEnvConfig,
        ...
        # Set terminal environment variables so whox tools pick them up.
    ...
    async def compute_reward(
        ...
        - ctx: ToolContext -- call ANY whox-agent tool (terminal, file, web,
