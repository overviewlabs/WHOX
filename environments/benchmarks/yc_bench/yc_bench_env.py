...
The evaluate flow:
    ...
    2. evaluate()  -- Iterates over all runs sequentially through:
        a. rollout_and_score_eval()  -- Per-run agent loop
            ...
            - Runs WHOXAgentLoop with terminal tool only
...
Requires: pip install whox-agent[yc-bench]
"""
...
class YCBenchEvalConfig(WHOXAgentEnvConfig):
    """
    Configuration for the YC-Bench evaluation environment.

    Extends WHOXAgentEnvConfig with YC-Bench-specific settings for
...
class YCBenchEvalEnv(WHOXAgentBaseEnv):
    """
    ...
    a competing built-in agent loop). The WHOXAgentLoop then drives the
    ...
    async def setup(self):
        ...
        try:
            ...
            raise RuntimeError(
                ...
                '  pip install "whox-agent[yc-bench]"\n'
    ...
    async def rollout_and_score_eval(self, eval_item: Dict[str, Any]) -> Dict:
        ...
        3. Runs WHOXAgentLoop with terminal tool
        ...
        try:
            ...
            # LiteLLM), which would compete with our WHOXAgentLoop.
