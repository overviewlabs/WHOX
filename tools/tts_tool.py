...
Configuration is loaded from ~/.whox/config.yaml under the 'tts:' key.
...
def _get_default_output_dir() -> str:
    ...
    return str(get_whox_dir("cache/audio", "audio_cache"))
...
# ===========================================================================
# Config loader -- reads tts: section from ~/.whox/config.yaml
# ===========================================================================
def _load_tts_config() -> Dict[str, Any]:
    """
    Load TTS configuration from ~/.whox/config.yaml.
    ...
    try:
        ...
        logger.debug("whox_cli.config not available, using default TTS config")
        return {}
...
def text_to_speech_tool(
    ...
    Reads provider/voice config from ~/.whox/config.yaml (tts: section).
    ...
    platform = os.getenv("WHOX_SESSION_PLATFORM", "").lower()
            ...
            if not _check_neutts_available():
                return json.dumps({
                    ...
                    "error": "NeuTTS provider selected but neutts is not installed. "
                             "Run whox setup and choose NeuTTS, or install espeak-ng and run python -m pip install -U neutts[all]."
                }, ensure_ascii=False)
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                "type": "string",
                "description": "Optional custom file path to save the audio. Defaults to ~/.whox/audio_cache/<timestamp>.mp3"
            }
        },
        "required": ["text"]
    }
