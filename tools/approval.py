...
def get_current_session_key(default: str = "default") -> str:
    ...
    if session_key:
        return session_key
    return os.getenv("WHOX_SESSION_KEY", default)

# Sensitive write targets that should trigger approval even when referenced
# via shell expansions like $HOME or $WHOX_HOME.
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_WHOX_ENV_PATH = (
    r'(?:~\/\.whox/|'
    r'(?:\$home|\$\{home\})/\.whox/|'
    r'(?:\$whox_home|\$\{whox_home\})/)'
    r'\.env\b'
)
...
DANGEROUS_PATTERNS = [
    ...
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart whox-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart whox-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(whox|gateway|cli\.py)\b', "kill whox/gateway process (self-termination)"),
...
def check_dangerous_command(command: str, env_type: str,
    ...
    is_cli = os.getenv("WHOX_INTERACTIVE")
    is_gateway = os.getenv("WHOX_GATEWAY_SESSION")

    if not is_cli and not is_gateway:
        return {"approved": True, "message": None}

    if is_gateway or os.getenv("WHOX_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    if os.getenv("WHOX_YOLO_MODE") or approval_mode == "off":
        return {"approved": True, "message": None}

    is_cli = os.getenv("WHOX_INTERACTIVE")
    is_gateway = os.getenv("WHOX_GATEWAY_SESSION")
    is_ask = os.getenv("WHOX_EXEC_ASK")
