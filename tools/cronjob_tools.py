"""
Cron job management tools for WHOX Agent.
...
def _origin_from_env() -> Optional[Dict[str, str]]:
    origin_platform = os.getenv("WHOX_SESSION_PLATFORM")
    origin_chat_id = os.getenv("WHOX_SESSION_CHAT_ID")
    if origin_platform and origin_chat_id:
        return {
            "platform": origin_platform,
            "chat_id": origin_chat_id,
            "chat_name": os.getenv("WHOX_SESSION_CHAT_NAME"),
            "thread_id": os.getenv("WHOX_SESSION_THREAD_ID"),
        }
    return None
...
- "in 2 minutes show me recent news in Hamilton" -> create (schedule='2m')
- "tomorrow at 9am remind me to call mom" -> create (schedule='YYYY-MM-DDT09:00:00')
- "every weekday at 8am send standup reminder" -> create (schedule='0 8 * * 1-5')
- "every Friday summarize this week's issues" -> create (recurring schedule)

If the user asks for a future or recurring action, create a cron job instead of
executing immediately in the current turn.
...
Important safety rule: cron-run sessions should not recursively schedule more cron jobs.""",
    "parameters": {
        ...
        "properties": {
            ...
            "script": {
                "type": "string",
                "description": "Optional path to a Python script that runs before each cron job execution. Its stdout is injected into the prompt as context. Use for data collection and change detection. Relative paths resolve under ~/.whox/scripts/. On update, pass empty string to clear."
            }
        },
        "required": ["action"]
    }
...
def check_cronjob_requirements() -> bool:
    ...
    """
    return bool(
        os.getenv("WHOX_INTERACTIVE")
        or os.getenv("WHOX_GATEWAY_SESSION")
        or os.getenv("WHOX_EXEC_ASK")
    )
