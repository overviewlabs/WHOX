...
- Persistent filesystems do NOT guarantee the same live sandbox or long-running processes survive cleanup, idle reaping, or WHOX exit
...
# display_whox_home imported lazily at call site (stale-module safety during whox update)
...
def _check_disk_usage_warning():
    ...
    try:
        ...
        # Get total size of whox directories
        total_bytes = 0
        import glob
        for path in glob.glob(str(scratch_dir / "whox-*")):
...
def _handle_sudo_failure(output: str, env_type: str) -> str:
    ...
    """
    is_gateway = os.getenv("WHOX_GATEWAY_SESSION")
...
def _prompt_for_sudo_password(timeout_seconds: int = 45) -> str:
    ...
    Only works in interactive mode (WHOX_INTERACTIVE=1).
    ...
    try:
        os.environ["WHOX_SPINNER_PAUSE"] = "1"
        ...
        if "WHOX_SPINNER_PAUSE" in os.environ:
            del os.environ["WHOX_SPINNER_PAUSE"]
...
def _transform_sudo_command(command: str) -> tuple[str, str | None]:
    ...
    If SUDO_PASSWORD is not set and in interactive mode (WHOX_INTERACTIVE=1):
...
def _parse_env_var(name: str, default: str, converter=int, type_label: str = "integer"):
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Check ~/.whox/.env or environment variables."
        )
        ...
        if modal_state["selected_backend"] != "direct":
            if modal_state["managed_mode_blocked"]:
                raise ValueError(
                    ...
                    "WHOX_ENABLE_NOUS_MANAGED_TOOLS is not enabled and no direct "
        ...
        if background:
            ...
            try:
                ...
                if check_interval and background:
                    ...
                    watcher_platform = os.getenv("WHOX_SESSION_PLATFORM", "")
                    watcher_chat_id = os.getenv("WHOX_SESSION_CHAT_ID", "")
                    watcher_thread_id = os.getenv("WHOX_SESSION_THREAD_ID", "")
