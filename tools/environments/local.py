...
_OUTPUT_FENCE = "__WHOX_FENCE_a9f7b3__"

# WHOX-internal env vars that should NOT leak into terminal subprocesses.
# These are loaded from ~/.whox/.env for WHOX' own LLM/provider calls
# but can break external CLIs (e.g. codex) that also honor them.
# See: https://github.com/NousResearch/whox-agent/issues/1002
#
...
_WHOX_PROVIDER_ENV_FORCE_PREFIX = "_WHOX_FORCE_"
...
def _build_provider_env_blocklist() -> frozenset:
    ...
    optional config registry, so new WHOX-managed secrets are blocked
    ...
    try:
        from whox_cli.config import OPTIONAL_ENV_VARS
    ...
    # Vars not covered above but still WHOX-internal / conflict-prone.
    blocked.update({
        "OPENAI_BASE_URL",
        "OPENAI_API_KEY",
...
_WHOX_PROVIDER_ENV_BLOCKLIST = _build_provider_env_blocklist()


def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    """Filter WHOX-managed secrets from a subprocess environment.

    `_WHOX_FORCE_<VAR>` entries in ``extra_env`` opt a blocked variable back in
    ...
    for key, value in (base_env or {}).items():
        if key.startswith(_WHOX_PROVIDER_ENV_FORCE_PREFIX):
            continue
        if key not in _WHOX_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value

    for key, value in (extra_env or {}).items():
        if key.startswith(_WHOX_PROVIDER_ENV_FORCE_PREFIX):
            real_key = key[len(_WHOX_PROVIDER_ENV_FORCE_PREFIX):]
            sanitized[real_key] = value
        elif key not in _WHOX_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value
...
def _find_bash() -> str:
    ...
    custom = os.environ.get("WHOX_GIT_BASH_PATH")
    ...
    raise RuntimeError(
        "Git Bash not found. WHOX Agent requires Git for Windows on Windows.\n"
        "Install it from: https://git-scm.com/download/win\n"
        "Or set WHOX_GIT_BASH_PATH to your bash.exe location."
    )
...
def _make_run_env(env: dict) -> dict:
    ...
    for k, v in merged.items():
        if k.startswith(_WHOX_PROVIDER_ENV_FORCE_PREFIX):
            real_key = k[len(_WHOX_PROVIDER_ENV_FORCE_PREFIX):]
            run_env[real_key] = v
        elif k not in _WHOX_PROVIDER_ENV_BLOCKLIST or _is_passthrough(k):
            run_env[k] = v
...
class LocalEnvironment(PersistentShellMixin, BaseEnvironment):
    ...
    def _temp_prefix(self) -> str:
        return f"/tmp/whox-local-{self._session_id}"
    ...
    def _execute_oneshot(self, command: str, cwd: str = "", *,
        ...
        user_shell = _find_bash()
        # Newline-separated wrapper (not `cmd; __whox_rc=...` on one line).
        # A trailing `; __whox_rc` glued to `<<EOF` / a closing `EOF` line breaks
