...
def _load_whox_env_vars() -> dict[str, str]:
    """Load ~/.whox/.env values without failing Docker command execution."""
    try:
        from whox_cli.config import load_env
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.whox/sandboxes/). Non-persistent
        ...
        container_name = f"whox-{uuid.uuid4().hex[:8]}"
        run_cmd = [
    ...
    def execute(self, command: str, cwd: str = "", *,
        ...
        try:
            ...
            pass
        whox_env = _load_whox_env_vars() if forward_keys else {}
        for key in sorted(forward_keys):
            value = os.getenv(key)
            if value is None:
                value = whox_env.get(key)
