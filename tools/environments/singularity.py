...
_SNAPSHOT_STORE = get_whox_home() / "singularity_snapshots.json"
...
def _get_scratch_dir() -> Path:
    ...
    Resolution order:
      ...
      4. ~/.whox/sandboxes/singularity (fallback)
    """
    ...
    if scratch.exists() and os.access(scratch, os.W_OK):
        user_scratch = scratch / os.getenv("USER", "whox") / "whox-agent"
...
class SingularityEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        self.instance_id = f"whox_{uuid.uuid4().hex[:12]}"
        ...
        if self._persistent:
            overlay_base = _get_scratch_dir() / "whox-overlays"
