...
wrapper, while preserving WHOX' persistent snapshot behavior across sessions.
"""
...
_SNAPSHOT_STORE = get_whox_home() / "modal_snapshots.json"
...
class ModalEnvironment(BaseModalExecutionEnvironment):
    ...
    def __init__(
        ...
        async def _create_sandbox(image_spec: Any):
            app = await _modal.App.lookup.aio("whox-agent", create_if_missing=True)
