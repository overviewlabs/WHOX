...
from whox_constants import get_whox_home, get_whox_dir
...
def _get_command_timeout() -> int:
    ...
    try:
        ...
        config_path = whox_home / "config.yaml"
...
def _socket_safe_tmpdir() -> str:
    ...
    append ``agent-browser-whox_…`` the resulting socket path exceeds the
...
BROWSER_TOOL_SCHEMAS = [
    ...
    },
    {
        ...
        "description": "Download a file URL discovered while browsing and save it locally. Returns a local file path that can be shared with users via MEDIA:<path>. Works for PDFs, images, zips, docs, and other direct file URLs.",
        "parameters": {
            "type": "object",
            "properties": {
                "url": {
                ...
                "filename": {
                    "type": "string",
                    "description": "Optional output filename override (e.g., 'report.pdf'). If omitted, WHOX derives it from URL/content-type."
                }
            },
            "required": ["url"]
        }
...
def _find_agent_browser() -> str:
    ...
    Checks in order: current PATH, Homebrew/common bin dirs, WHOX-managed
    ...
    # Build an extended search PATH including Homebrew and WHOX-managed dirs.
    ...
    whox_home = get_whox_home()
    whox_node_bin = str(whox_home / "node" / "bin")
    if os.path.isdir(whox_node_bin):
        extra_dirs.append(whox_node_bin)
...
def _run_browser_command(
    ...
    try:
        ...
        # Ensure PATH includes WHOX-managed Node first, Homebrew versioned
...
def _maybe_start_recording(task_id: str):
    ...
    try:
        ...
        recordings_dir = whox_home / "browser_recordings"
...
def _derive_download_filename(url: str, filename: Optional[str], content_type: str) -> str:
    """
    Derive a safe output filename from user input, URL path, and content type.
    """
    ...
    # Keep filenames portable and shell-safe.
    candidate = re.sub(r"[^A-Za-z0-9._-]+", "_", candidate).strip("._")
    if not candidate:
        candidate = f"download_{int(time.time())}"

    # Add extension if missing and content type suggests one.
    ...
    # Bound length while preserving extension.
    if len(candidate) > 180:
        stem, ext = os.path.splitext(candidate)
        candidate = f"{stem[:160]}{ext[:20]}"

    return candidate or f"download_{int(time.time())}.bin"


def browser_download(url: str, filename: Optional[str] = None, task_id: Optional[str] = None) -> str:
    """
    Download a file URL discovered during browsing and save it to local cache.
    ...
    Returns:
        JSON string with saved path and metadata.
    """
    from agent.redact import _PREFIX_RE
    ...
    if not _is_local_backend() and not _allow_private_urls() and not _is_safe_url(target):
        return json.dumps({
            "success": False,
            "error": "Blocked: URL targets a private or internal address",
        }, ensure_ascii=False)
    ...
    effective_task_id = task_id or "default"
    downloads_dir = get_whox_dir("cache/downloads", "browser_downloads")
    downloads_dir.mkdir(parents=True, exist_ok=True)

    # Optional referer header from active browser page for sites that require it.
    referer = None
    try:
        if effective_task_id in _active_sessions:
            ref_result = _run_browser_command(effective_task_id, "eval", ["window.location.href"])
            if ref_result.get("success"):
                raw = ref_result.get("data", {}).get("result", "")
                if isinstance(raw, str):
                    referer = raw.strip().strip('"').strip("'")
    except Exception:
        referer = None

    headers = {
        "User-Agent": os.environ.get(
            "BROWSER_DOWNLOAD_USER_AGENT",
    ...
    max_mb = max(1, int(os.environ.get("BROWSER_DOWNLOAD_MAX_MB", "250")))
    max_bytes = max_mb * 1024 * 1024

    tmp_path: Optional[Path] = None
    try:
        with requests.get(target, headers=headers, stream=True, timeout=(10, 120), allow_redirects=True) as resp:
            resp.raise_for_status()
            final_url = str(resp.url or target)

            # Post-redirect checks.
            if not _is_local_backend() and not _allow_private_urls() and not _is_safe_url(final_url):
                return json.dumps({
                    "success": False,
            ...
            out_name = _derive_download_filename(final_url, filename, content_type)
            out_path = downloads_dir / out_name
            if out_path.exists():
                stem, ext = os.path.splitext(out_name)
                out_path = downloads_dir / f"{stem}_{int(time.time())}{ext}"

            tmp_path = downloads_dir / f".{out_path.name}.part"
            total = 0
            with open(tmp_path, "wb") as f:
                for chunk in resp.iter_content(chunk_size=65536):
                    ...
                    total += len(chunk)
                    if total > max_bytes:
                        raise ValueError(f"Download exceeds max size limit ({max_mb} MB)")
                    f.write(chunk)
        ...
        return json.dumps({
            ...
            "share_hint": f"Use MEDIA:{out_path} to send this file to the user.",
        }, ensure_ascii=False)

    except requests.Timeout:
        return json.dumps({"success": False, "error": "Download timed out"}, ensure_ascii=False)
    except requests.HTTPError as e:
        code = getattr(getattr(e, "response", None), "status_code", None)
        return json.dumps({"success": False, "error": f"HTTP error while downloading ({code})"}, ensure_ascii=False)
    except Exception as e:
        logger.warning("browser_download failed for %s: %s", target, e)
        return json.dumps({"success": False, "error": f"Download failed: {str(e)}"}, ensure_ascii=False)
    finally:
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> str:
    ...
    from whox_constants import get_whox_dir
    screenshots_dir = get_whox_dir("cache/screenshots", "browser_screenshots")
