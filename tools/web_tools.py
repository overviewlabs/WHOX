...
Backend is selected during ``whox tools`` setup (web.backend in config.yaml).
When available, WHOX can route Firecrawl calls through a Nous-hosted tool-gateway
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.whox/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``whox tools``).
...
def _raise_web_backend_configuration_error() -> None:
    ...
    if managed_nous_tools_enabled():
        message += (
            ...
            "(`whox model`) and provide FIRECRAWL_GATEWAY_URL or TOOL_GATEWAY_DOMAIN."
        )
...
def _get_firecrawl_client():
    ...
    WHOX falls back to the Firecrawl tool-gateway for logged-in Nous Subscribers.
    """
...
def _get_firecrawl_http_config() -> tuple[str, Dict[str, str]]:
    """Resolve Firecrawl base URL + headers for direct HTTP calls."""
    direct_config = _get_direct_firecrawl_config()
    if direct_config is not None:
        ...
        if managed_gateway is None:
            logger.error("Firecrawl HTTP config failed: missing direct config and tool-gateway auth.")
            _raise_web_backend_configuration_error()
        kwargs = {
            "api_key": managed_gateway.nous_user_token,
            "api_url": managed_gateway.gateway_origin,
    ...
    base_url = (kwargs.get("api_url") or "").rstrip("/")
    if not base_url:
        _raise_web_backend_configuration_error()
...
def _firecrawl_v1_search(query: str, limit: int, category: str = "general") -> dict:
    """Call Firecrawl v1 search endpoint directly."""
...
def _searxng_search(query: str, category: str, limit: int, pageno: int = 1) -> Dict[str, Any]:
    """
    Run direct SearXNG search using category + pageno parameters.
    """
    base_url = _get_searxng_base_url()
    if not base_url:
        raise ValueError("SEARXNG_API_URL is not configured")
...
def _firecrawl_v1_scrape(url: str, formats: List[str]) -> dict:
    """Call Firecrawl v1 scrape endpoint directly."""
    base_url, headers = _get_firecrawl_http_config()
...
def _is_actual_image_url(url: str) -> bool:
    """Best-effort validation that URL points to an image resource."""
    ...
    try:
        with httpx.Client(timeout=10.0, follow_redirects=True) as client:
            ...
            if ctype.startswith("image/"):
                return True
            # Some origins don't support HEAD well.
            if response.status_code in (405, 403) or not ctype:
                get_resp = client.get(url, headers={"Range": "bytes=0-0"})
                get_ctype = (get_resp.headers.get("content-type") or "").lower()
                if get_ctype.startswith("image/"):
                    return True
    except Exception:
...
def _extract_image_src_candidates(item: Dict[str, Any]) -> List[str]:
    candidates: List[str] = []
...
def _shape_image_search_results(web_results: List[Dict[str, Any]], limit: int) -> List[Dict[str, Any]]:
    """Return image search results as src-only entries for actual image files."""
    ...
    for item in web_results:
        ...
        for candidate in _extract_image_src_candidates(item):
            if candidate in seen:
                continue
            if not _is_actual_image_url(candidate):
                continue
            seen.add(candidate)
            images.append({
                "src": candidate,
...
def _extract_image_urls_from_scrape_payload(payload: Dict[str, Any]) -> List[str]:
    """Extract candidate image URLs from Firecrawl scrape payload."""
    candidates: List[str] = []
    if not isinstance(payload, dict):
        return candidates

    meta = payload.get("metadata") if isinstance(payload.get("metadata"), dict) else {}
    for key in ("ogImage", "og:image", "twitter:image", "image", "image:url"):
        value = meta.get(key)
        if isinstance(value, str) and value.strip():
            candidates.append(value.strip())
    ...
    if isinstance(markdown, str) and markdown:
        ...
        candidates.extend(re.findall(md_pattern, markdown))
        candidates.extend(re.findall(html_pattern, markdown))
...
def _resolve_web_extract_auxiliary(model: Optional[str] = None) -> tuple[Optional[Any], Optional[str], Dict[str, Any]]:
    ...
    if client is not None and _is_nous_auxiliary_client(client):
        ...
        extra_body = get_auxiliary_extra_body() or {"tags": ["product=whox-agent"]}
...
def _get_exa_client():
    ...
    if _exa_client is None:
        ...
        _exa_client.headers["x-exa-integration"] = "whox-agent"
    return _exa_client
...
def web_search_tool(query: str, limit: int = 5, category: str = "general", pageno: int = 1) -> str:
    ...
    try:
        ...
        # by fetching enough rows then slicing to requested page.
        fetch_limit = min(100, per_page * page)
        ...
        if backend == "parallel":
            if normalized_category != "general":
                return json.dumps({
                    "success": False,
                    "error": f"Backend '{backend}' only supports category='general' for web_search."
                }, ensure_ascii=False)
            response_data = _parallel_search(query, fetch_limit)
            web_rows = response_data.get("data", {}).get("web", [])
            page_rows = _paginate_rows(web_rows, per_page, page)
            response_data["data"]["web"] = page_rows
            response_data["category"] = normalized_category
            response_data["pageno"] = page
            response_data["per_page"] = per_page
            debug_call_data["results_count"] = len(page_rows)
        ...
        if backend == "exa":
            ...
            response_data = _exa_search(query, fetch_limit)
            web_rows = response_data.get("data", {}).get("web", [])
            page_rows = _paginate_rows(web_rows, per_page, page)
            response_data["data"]["web"] = page_rows
            response_data["category"] = normalized_category
        ...
        if backend == "tavily":
            if normalized_category not in ("general", "news"):
                return json.dumps({
                    "success": False,
                    "error": "Backend 'tavily' supports category='general' or category='news' only."
                }, ensure_ascii=False)
            logger.info("Tavily search: '%s' (limit: %d)", query, limit)
            raw = _tavily_request("search", {
                "query": query,
                "max_results": min(fetch_limit, 20),
                "include_raw_content": False,
                "include_images": False,
                "topic": "news" if normalized_category == "news" else "general",
            })
        ...
        if normalized_category == "images":
            ...
            if (not used_v2) and len(images) < fetch_limit:
                ...
                for row in web_results:
                    ...
                    try:
                        scrape_raw = _firecrawl_v1_scrape(page_url, ["markdown"])
                        scrape_payload = _extract_scrape_payload(scrape_raw)
                        extracted = _extract_image_urls_from_scrape_payload(scrape_payload)
                        for src in extracted:
                            if any(img.get("src") == src for img in images):
                                continue
                            if not _is_actual_image_url(src):
                                continue
                            images.append(
                                {
                                    "src": src,
                        ...
                        # Best-effort fallback only; keep going.
                        pass
                    if len(images) >= fetch_limit:
                        break
            page_images = _paginate_rows(images, per_page, page)
            response_data = {
                "success": True,
                "category": "images",
                "image_src_only": True,
                ...
                "data": {
                    ...
                    "web": [
                        ...
                        for idx, item in enumerate(page_images)
                    ],
                },
            }
            results_count = len(page_images)
        else:
            page_web = _paginate_rows(web_results, per_page, page)
            # Build response with standard search metadata.
            response_data = {
                "success": True,
                "category": normalized_category,
                "search_backend": search_backend_detail,
    ...
    try:
        ...
        if not safe_urls:
            ...
            if backend == "parallel":
                ...
                # ── Firecrawl extraction (v1 endpoint) ──
                # Determine requested formats for Firecrawl scrape
                formats: List[str] = []
                if format == "markdown":
                    formats = ["markdown"]
                elif format == "html":
                ...
                for url in safe_urls:
                    ...
                    try:
                        ...
                        scrape_result = _firecrawl_v1_scrape(
                            url=url,
                            formats=formats,
                        )
...
if __name__ == "__main__":
    ...
    if nous_available:
        print("\nLLM-enhanced usage:")
        print("  # Raw content by default (no LLM post-processing)")
        ...
        print("  )")
        print("")
        print("  # Enable LLM processing (optional)")
        print("  summarized = await web_extract_tool(['https://example.com'], use_llm_processing=True)")
...
WEB_EXTRACT_SCHEMA = {
    "name": "web_extract",
    "description": "Extract content from web page URLs. Returns page content in markdown format. Also works with PDF URLs (arxiv papers, documents, etc.) — pass the PDF link directly and it converts to markdown text. By default, raw page content is returned without LLM post-processing. Pages over 2M chars are refused. If a URL fails or times out, use the browser tool to access it instead.",
    "parameters": {
        "type": "object",
        "properties": {
            "urls": {
