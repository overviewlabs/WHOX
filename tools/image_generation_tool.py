...
VALID_IMAGE_PROVIDERS = ["auto", "fal", "cloudflare"]

_CLOUDFLARE_SIZE_MAP = {
    "landscape": {"width": 1024, "height": 576},
    "square": {"width": 1024, "height": 1024},
    "portrait": {"width": 576, "height": 1024},
}
_CLOUDFLARE_HIGH_QUALITY_SIZE_MAP = {
    "landscape": {"width": 1536, "height": 864},
    "square": {"width": 1536, "height": 1536},
    "portrait": {"width": 864, "height": 1536},
}

_QUALITY_PROMPT_PREFIX = (
    "ultra high quality, highly detailed, crisp textures, professional lighting, "
    "clean composition"
)
...
_last_image_by_task: Dict[str, str] = {}
_last_image_lock = threading.Lock()


def _cloudflare_account_id() -> str:
    return str(os.getenv("CLOUDFLARE_ACCOUNT_ID", "")).strip()


def _cloudflare_api_token() -> str:
    return str(os.getenv("CLOUDFLARE_API_TOKEN", "")).strip()


def _cloudflare_ready() -> bool:
    return bool(_cloudflare_account_id() and _cloudflare_api_token())


def _resolve_image_provider() -> str:
    requested = str(os.getenv("IMAGE_GEN_PROVIDER", "auto")).strip().lower() or "auto"
    if requested not in VALID_IMAGE_PROVIDERS:
        logger.warning(
            "Invalid IMAGE_GEN_PROVIDER '%s'. Falling back to auto mode.",
            requested,
        )
        requested = "auto"
...
def _cache_cloudflare_image_bytes(payload: bytes, content_type: str = "") -> str:
    from gateway.platforms.base import cache_image_from_bytes

    _type = str(content_type or "").lower()
    ext = ".png"
    if "jpeg" in _type or "jpg" in _type:
        ext = ".jpg"
    elif "webp" in _type:
        ext = ".webp"
    elif "gif" in _type:
        ext = ".gif"
    return cache_image_from_bytes(payload, ext=ext)


def _extract_cloudflare_image_url_or_path(resp_json: Dict[str, Any]) -> Optional[str]:
    result = resp_json.get("result")
    if isinstance(result, str):
        if result.startswith("http://") or result.startswith("https://"):
            return result
        try:
            return _cache_cloudflare_image_bytes(base64.b64decode(result), "image/png")
        except Exception:
            return None
    ...
    if isinstance(image_field, str):
        if image_field.startswith("http://") or image_field.startswith("https://"):
            return image_field
        try:
            return _cache_cloudflare_image_bytes(base64.b64decode(image_field), "image/png")
        except Exception:
            pass
    ...
    if isinstance(images, list) and images:
        ...
        if isinstance(first, str):
            if first.startswith("http://") or first.startswith("https://"):
                return first
            try:
                return _cache_cloudflare_image_bytes(base64.b64decode(first), "image/png")
            except Exception:
                return None
        if isinstance(first, dict):
            maybe_url = first.get("url") or first.get("image_url")
            if isinstance(maybe_url, str) and (maybe_url.startswith("http://") or maybe_url.startswith("https://")):
                return maybe_url
            maybe_b64 = first.get("b64_json") or first.get("image")
            if isinstance(maybe_b64, str):
                try:
                    return _cache_cloudflare_image_bytes(base64.b64decode(maybe_b64), "image/png")
                except Exception:
                    return None
    return None


def _resolve_cloudflare_input_image(input_image: str) -> tuple[str, bytes, str]:
    source = str(input_image or "").strip()
    if not source:
        raise ValueError("input_image cannot be empty")

    if source.startswith("data:"):
        match = re.match(r"^data:([^;,]+);base64,(.+)$", source, flags=re.IGNORECASE | re.DOTALL)
        if not match:
            raise ValueError("Unsupported data URL format for input_image")
        mime_type = match.group(1).strip().lower() or "image/png"
        payload = base64.b64decode(match.group(2))
        ext = mimetypes.guess_extension(mime_type) or ".png"
        return (f"input{ext}", payload, mime_type)

    if source.startswith("http://") or source.startswith("https://"):
        try:
            import httpx
        except Exception as exc:
            raise RuntimeError("httpx is required for URL-based input_image") from exc
        with httpx.Client(timeout=60.0, follow_redirects=True) as client:
            resp = client.get(source)
        if resp.status_code >= 400:
            raise RuntimeError(f"Failed to download input_image: HTTP {resp.status_code}")
        mime_type = str(resp.headers.get("content-type", "image/png")).split(";", 1)[0].strip().lower()
        ext = mimetypes.guess_extension(mime_type) or ".png"
        return (f"input{ext}", resp.content, mime_type)

    if not os.path.isfile(source):
        raise ValueError(f"input_image path not found: {source}")

    with open(source, "rb") as f:
        payload = f.read()
    mime_type = (mimetypes.guess_type(source)[0] or "image/png").lower()
    ext = os.path.splitext(source)[1] or (mimetypes.guess_extension(mime_type) or ".png")
    return (f"input{ext}", payload, mime_type)


def _quality_enhanced_prompt(prompt: str) -> str:
    clean = str(prompt or "").strip()
    if not clean:
        return _QUALITY_PROMPT_PREFIX
    return f"{_QUALITY_PROMPT_PREFIX}, {clean}"


def _quality_mode() -> str:
    mode = str(os.getenv("IMAGE_GEN_QUALITY_MODE", "balanced")).strip().lower() or "balanced"
    if mode not in {"balanced", "max"}:
        return "balanced"
    return mode


def _remember_last_image(task_id: Optional[str], image_ref: Optional[str]) -> None:
    ...
    with _last_image_lock:
        _last_image_by_task[task_id] = image_path


def _get_last_image(task_id: Optional[str]) -> Optional[str]:
    if not task_id:
        return None
    with _last_image_lock:
        candidate = _last_image_by_task.get(task_id)
...
def _looks_like_followup_edit(prompt: str) -> bool:
    ...
    if any(marker in p for marker in explicit_new):
        return False
    ...
    return any(marker in p for marker in followup_markers)
...
def _generate_with_cloudflare(
    ...
    if not _cloudflare_ready():
        raise ValueError(
            "Cloudflare image generation requires CLOUDFLARE_ACCOUNT_ID and CLOUDFLARE_API_TOKEN."
        )

    model = str(
        os.getenv("IMAGE_GEN_CLOUDFLARE_MODEL", DEFAULT_CLOUDFLARE_MODEL)
    ).strip() or DEFAULT_CLOUDFLARE_MODEL
    quality_mode = _quality_mode()
    size = _CLOUDFLARE_SIZE_MAP.get(
        aspect_ratio,
        _CLOUDFLARE_SIZE_MAP[DEFAULT_ASPECT_RATIO],
    )
    if quality_mode == "max":
        size = _CLOUDFLARE_HIGH_QUALITY_SIZE_MAP.get(
            aspect_ratio,
            _CLOUDFLARE_HIGH_QUALITY_SIZE_MAP[DEFAULT_ASPECT_RATIO],
        )
    endpoint = (
        f"https://api.cloudflare.com/client/v4/accounts/{_cloudflare_account_id()}"
        f"/ai/run/{model}"
    )
    ...
    try:
        ...
        raise RuntimeError("httpx is required for Cloudflare image generation") from exc

    # FLUX.2 klein/dev on Workers AI require multipart form input even for
    # prompt-only generation.
    def _call_cloudflare(size_map: Dict[str, int]):
        prompt_text = str(prompt or "").strip()
        if quality_mode == "max":
            prompt_text = _quality_enhanced_prompt(prompt_text)
        payload = {
            "prompt": prompt_text,
            "width": str(int(size_map["width"])),
            "height": str(int(size_map["height"])),
        }
        # klein models have fixed steps; don't send custom step count for them.
        if "klein" not in model:
            payload["steps"] = str(max(1, min(int(num_inference_steps), 50)))
        headers = {"Authorization": f"Bearer {_cloudflare_api_token()}"}
        multipart_fields = {k: (None, v) for k, v in payload.items()}
        if input_image:
            multipart_fields["input_image_0"] = _resolve_cloudflare_input_image(input_image)
        with httpx.Client(timeout=90.0) as client:
            return client.post(endpoint, headers=headers, files=multipart_fields)

    resp = _call_cloudflare(size)

    if resp.status_code >= 400:
        snippet = resp.text[:500]
        raise RuntimeError(f"Cloudflare Workers AI error {resp.status_code}: {snippet}")

    content_type = str(resp.headers.get("content-type", "")).lower()
    if content_type.startswith("image/"):
        image_ref = _cache_cloudflare_image_bytes(resp.content, content_type)
        return json.dumps({"success": True, "image": image_ref}, indent=2, ensure_ascii=False)

    data = resp.json()
    if data.get("success") is False:
        errs = data.get("errors") or []
        raise RuntimeError(f"Cloudflare Workers AI returned success=false: {errs}")

    image_ref = _extract_cloudflare_image_url_or_path(data)
    if not image_ref:
        raise RuntimeError("Cloudflare Workers AI response did not contain a usable image")
...
def image_generate_tool(
    ...
    try:
        ...
        logger.info("Generating %s image(s) with provider=%s: %s", num_images, provider, prompt[:80])
        
        # Validate prompt
        if not prompt or not isinstance(prompt, str) or len(prompt.strip()) == 0:
            raise ValueError("Prompt is required and must be a non-empty string")

        # Conversation continuity: when the user asks for a follow-up edit
        # ("make it black", "keep same dog but...") and no input_image was
        # provided, reuse the last image generated in this same task/session.
        effective_input_image = input_image
        if not effective_input_image and _looks_like_followup_edit(prompt):
            effective_input_image = _get_last_image(task_id)
        
        if provider == "cloudflare":
            result = _generate_with_cloudflare(
                prompt=prompt,
                aspect_ratio=aspect_ratio_lower,
                num_inference_steps=num_inference_steps,
                input_image=effective_input_image,
            )
            try:
                _img = json.loads(result).get("image")
            except Exception:
                _img = None
            _remember_last_image(task_id, _img)
            debug_call_data["success"] = True
            debug_call_data["images_generated"] = 1
            debug_call_data["generation_time"] = (datetime.datetime.now() - start_time).total_seconds()
            _debug.log_call("image_generate_tool", debug_call_data)
            _debug.save()
            return result

        # FAL mode: Check API key availability
        ...
        arguments = {
            "prompt": _quality_enhanced_prompt(prompt) if _quality_mode() == "max" else prompt.strip(),
        ...
        _remember_last_image(task_id, response_data.get("image"))
...
def check_image_generation_requirements() -> bool:
    ...
    try:
        provider = _resolve_image_provider()
        if provider == "cloudflare":
            if not check_cloudflare_requirements():
                return False
...
IMAGE_GENERATE_SCHEMA = {
    "name": "image_generate",
    "description": "Generate images from prompts, and edit an existing image by passing input_image (local path, URL, or data URL). For Telegram edits, pass the MEDIA path from the user message as input_image.",
    "parameters": {
        "type": "object",
        "properties": {
            "prompt": {
            ...
            "input_image": {
                "type": "string",
                "description": "Optional source image for editing/variation. Accepts local file path, http(s) URL, or data URL. When set, model will edit/transform this image guided by prompt."
            },
            "aspect_ratio": {
                "type": "string",
