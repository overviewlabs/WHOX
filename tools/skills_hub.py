#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the WHOX Skills Hub.
...
Used by whox_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            whox_meta = metadata.get("whox", {})
            if isinstance(whox_meta, dict):
                tags = whox_meta.get("tags", [])
        if not tags:
...
class OptionalSkillSource(SkillSource):
    ...
    def __init__(self):
        from whox_constants import get_optional_skills_dir
