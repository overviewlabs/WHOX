...
it is automatically downloaded from GitHub releases to $WHOX_HOME/bin/tirith.
...
def _get_whox_home() -> str:
    """Return the WHOX home directory, respecting WHOX_HOME env var."""
    return str(get_whox_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_whox_home(), ".tirith-install-failed")
...
def _whox_bin_dir() -> str:
    """Return $WHOX_HOME/bin, creating it if needed."""
    d = os.path.join(_get_whox_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $WHOX_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_whox_bin_dir(), "tirith")
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $WHOX_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $WHOX_HOME/bin/tirith
    ...
    whox_bin = os.path.join(_whox_bin_dir(), "tirith")
    if os.path.isfile(whox_bin) and os.access(whox_bin, os.X_OK):
        _resolved_path = whox_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return whox_bin
