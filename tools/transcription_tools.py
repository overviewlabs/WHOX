...
LOCAL_STT_COMMAND_ENV = "WHOX_LOCAL_STT_COMMAND"
LOCAL_STT_LANGUAGE_ENV = "WHOX_LOCAL_STT_LANGUAGE"
...
def get_stt_model_from_config() -> Optional[str]:
    """Read the STT model name from ~/.whox/config.yaml.
    ...
    try:
        import yaml
        cfg_path = get_whox_home() / "config.yaml"
        if cfg_path.exists():
    ...
    if explicit:
        if provider == "local":
            ...
            logger.warning(
                ...
                "(install faster-whisper or set WHOX_LOCAL_STT_COMMAND)"
            )
            return "none"
...
def _transcribe_local_command(file_path: str, model_name: str) -> Dict[str, Any]:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="whox-local-stt-") as output_dir:
