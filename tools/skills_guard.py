...
Trust levels:
  - builtin:   Ships with WHOX. Never scanned, always trusted.
...
THREAT_PATTERNS = [
    ...
    (r'\$HOME/\.whox/\.env|\~/\.whox/\.env',
     "whox_env_access", "critical", "exfiltration",
     "directly references WHOX secrets file"),
    ...
    (r'\.whox/config\.yaml|\.whox/SOUL\.md',
     "whox_config_mod", "critical", "persistence",
     "references WHOX configuration files directly"),
...
def _get_configured_model() -> str:
    """Load the user's configured model from ~/.whox/config.yaml."""
    try:
