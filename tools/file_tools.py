...
def read_file_tool(path: str, offset: int = 1, limit: int = 500, task_id: str = "default") -> str:
    ...
    try:
        ...
        # ── WHOX internal path guard ────────────────────────────────
        # Prevent prompt injection via catalog or hub metadata files.
        import pathlib as _pathlib
        from whox_constants import get_whox_home as _get_hh
        _resolved = _pathlib.Path(path).expanduser().resolve()
        _whox_home = _get_hh().resolve()
        _blocked_dirs = [
            _whox_home / "skills" / ".hub" / "index-cache",
            _whox_home / "skills" / ".hub",
        ]
        for _blocked in _blocked_dirs:
            try:
                ...
                return json.dumps({
                    "error": (
                        f"Access denied: {path} is an internal WHOX cache file "
