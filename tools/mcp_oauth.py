...
``httpx.Auth``) with WHOX-specific token storage and browser-based
...
# ---------------------------------------------------------------------------
# Token storage — persists tokens + client info to ~/.whox/mcp-tokens/
# ---------------------------------------------------------------------------
...
class WHOXTokenStorage:
    ...
    def _base_dir(self) -> Path:
        home = Path(os.environ.get("WHOX_HOME", Path.home() / ".whox"))
...
async def _wait_for_callback() -> tuple[str, str | None]:
    ...
    connecting and potentially hanging WHOX startup indefinitely.
    """
    global _oauth_port
    ...
    if not code:
        raise OAuthNonInteractiveError(
            ...
            "Run 'whox mcp auth <server-name>' to authorize interactively."
        )
    return code, state
...
def build_oauth_auth(server_name: str, server_url: str):
    ...
    storage = WHOXTokenStorage(server_name)
    ...
    if not interactive:
        ...
        if not has_cached:
            logger.warning(
                ...
                "connect. Run 'whox mcp auth %s' to authorize interactively.",
    ...
    if not interactive:
        ...
        async def _noninteractive_callback() -> tuple[str, str | None]:
            raise OAuthNonInteractiveError(
                ...
                f"(no TTY). Run 'whox mcp auth {server_name}' to "
...
def remove_oauth_tokens(server_name: str) -> None:
    ...
    WHOXTokenStorage(server_name).remove()
