...
```bash
whox memory setup    # select "byterover"
```

Or manually:
```bash
whox config set memory.provider byterover
# Optional cloud sync:
echo "BRV_API_KEY=your-key" >> ~/.whox/.env
```
...
Working directory: `$WHOX_HOME/byterover/` (profile-scoped).
