...
```bash
whox memory setup    # select "hindsight"
```
...
```bash
whox config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.whox/.env
```
...
Daemon startup logs: `~/.whox/logs/hindsight-embed.log`
Daemon runtime logs: `~/.hindsight/profiles/<profile>.log`

## Config

Config file: `~/.whox/hindsight/config.json`
...
The LLM API key is stored in `~/.whox/.env` as `HINDSIGHT_LLM_API_KEY`.
