...
Config via environment variables:
  ...
  HINDSIGHT_BANK_ID   — memory bank identifier (default: whox)
  HINDSIGHT_BUDGET    — recall budget: low/mid/high (default: mid)
  HINDSIGHT_API_URL   — API endpoint
  HINDSIGHT_MODE      — cloud or local (default: cloud)

Or via $WHOX_HOME/hindsight/config.json (profile-scoped), falling back to
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $WHOX_HOME/hindsight/config.json  (profile-scoped)
    ...
    profile_path = get_whox_home() / "hindsight" / "config.json"
    ...
    return {
        ...
        "banks": {
            "whox": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "whox"),
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, whox_home):
        """Write config to $WHOX_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
        config_dir = Path(whox_home) / "hindsight"
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name", "default": "whox"},
        ...
        if self._client is None:
            if self._mode == "local":
                ...
                self._client = HindsightEmbedded(
                    profile=self._config.get("profile", "whox"),
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        banks = self._config.get("banks", {}).get("whox", {})
        self._bank_id = self._config.get("bank_id") or banks.get("bankId", "whox")
        ...
        if self._mode == "local":
            def _start_daemon():
                import traceback
                from pathlib import Path
                log_dir = Path(os.environ.get("WHOX_HOME", os.path.expanduser("~/.whox"))) / "logs"
