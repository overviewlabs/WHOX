...
```bash
whox memory setup    # select "holographic"
```

Or manually:
```bash
whox config set memory.provider holographic
```

## Config

Config in `config.yaml` under `plugins.whox-memory-store`:

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$WHOX_HOME/memory_store.db` | SQLite database path |
