...
Resolution order for config file:
  1. $WHOX_HOME/honcho.json  (instance-local, enables isolated WHOX instances)
...
def resolve_active_host() -> str:
    """Derive the Honcho host key from the active WHOX profile.

    Resolution order:
      1. WHOX_HONCHO_HOST env var (explicit override)
      2. Active profile name via profiles system -> ``whox.<profile>``
      3. Fallback: ``"whox"`` (default profile)
    """
    explicit = os.environ.get("WHOX_HONCHO_HOST", "").strip()
    if explicit:
        return explicit
...
def resolve_config_path() -> Path:
    ...
    Resolution order:
      1. $WHOX_HOME/honcho.json      (profile-local, if it exists)
      2. ~/.whox/honcho.json          (default profile — shared host blocks live here)
      3. ~/.honcho/config.json          (global, cross-app interop)

    Returns the global path if none exist (for first-time setup writes).
    """
    local_path = get_whox_home() / "honcho.json"
    if local_path.exists():
        return local_path

    # Default profile's config — host blocks accumulate here via setup/clone
    default_path = Path.home() / ".whox" / "honcho.json"
...
def _resolve_memory_mode(
    ...
    Object form:  { "default": "hybrid", "whox": "honcho", ... }
...
class HonchoClientConfig:
    ...
    # Config object form: "memoryMode": { "default": "hybrid", "whox": "honcho" }
    ...
    # Max chars of dialectic result to inject into WHOX system prompt
    ...
    # True when Honcho was explicitly configured for this host (hosts.whox
    ...
    def from_global_config(
        ...
        Resolution: $WHOX_HOME/honcho.json -> ~/.honcho/config.json -> env vars.
        When host is None, derives it from the active WHOX profile.
        """
        resolved_host = host or resolve_active_host()
        ...
        # A hosts.whox block or explicit enabled flag means the user
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. WHOX session title (from /title command)
          3. per-session strategy — WHOX session_id ({timestamp}_{hex})
        ...
        # per-session: inherit WHOX session_id (new Honcho session each run)
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    if not config.api_key and not config.base_url:
        raise ValueError(
            ...
            "then run 'whox honcho setup' or set HONCHO_API_KEY. "
    ...
    if not resolved_base_url:
        try:
            ...
            whox_cfg = load_config()
            honcho_cfg = whox_cfg.get("honcho", {})
