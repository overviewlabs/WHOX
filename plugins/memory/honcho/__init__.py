...
Config: Uses the existing Honcho config chain:
  1. $WHOX_HOME/honcho.json (profile-scoped)
...
class HonchoMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, whox_home):
        """Write config to $WHOX_HOME/honcho.json (Honcho SDK native format)."""
        import json
        from pathlib import Path
        config_path = Path(whox_home) / "honcho.json"
        existing = {}
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        try:
            ...
            try:
                whox_home = kwargs.get("whox_home", "")
                if whox_home and not cfg.raw.get("aiPeer"):
                    soul_path = Path(whox_home) / "SOUL.md"
                    if soul_path.exists():
    ...
    def _do_session_init(self, cfg, session_id: str, **kwargs) -> None:
        ...
        try:
            if not session.messages:
                ...
                mem_dir = str(get_whox_home() / "memories")
    ...
    def _ensure_session(self) -> bool:
        ...
        try:
            self._do_session_init(
                self._config,
                self._lazy_init_session_id or "whox-default",
