...
version: 1.0.0
author: arceus77-7, enhanced by WHOX Agent
license: MIT
metadata:
  whox:
...
- `tmux` available for stable authenticated sessions during WHOX terminal calls (desktop app flow only)
...
### Service Account (recommended for WHOX)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.whox/.env` (the skill will prompt for this on first load).
...
## WHOX Execution Pattern (desktop app flow)

WHOX terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/whox-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/whox-op.sock"
