---
name: telephony
description: Give WHOX phone capabilities without core tool changes. Provision and persist a Twilio number, send and receive SMS/MMS, make direct calls, and place AI-driven outbound calls through Bland.ai or Vapi.
version: 1.0.0
...
- save provider credentials into `~/.whox/.env`
...
4. Treat third-party phone numbers as sensitive operational data:
   - do not save them to WHOX memory
...
### 1) "I want WHOX to own a real phone number"
Use **Twilio**.
...
- pairs well with WHOX `text_to_speech` plus a public file host or tunnel
...
3. Buy it and save it into `~/.whox/.env` + state:
```bash
python3 "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
Use this when:
- you want the call to use WHOX's configured TTS voice rather than Twilio `<Say>`
...
1. Generate the audio with WHOX `text_to_speech`.
2. Make the resulting MP3 publicly reachable.
3. Place the Twilio call with `--audio-url`.

Example agent flow:
- Ask WHOX to create the message audio with `text_to_speech`
...
Important note:
- WHOX TTS is great for prerecorded outbound messages
...
- WHOX STT/TTS alone is not being used here as a full duplex phone conversation engine; that would require a much heavier streaming/webhook integration than this skill is trying to introduce
...
7. Summarize the outcome without persisting third-party numbers to WHOX memory.
...
- Do not store arbitrary third-party phone numbers in WHOX memory.
...
3. persist that number to `~/.whox/.env`
