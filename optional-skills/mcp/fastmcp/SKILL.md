...
- smoke-test a server with the FastMCP CLI before wiring it into WHOX or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to WHOX. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.whox/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.whox/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is WHOX integration, either:

- configure the server in `~/.whox/config.yaml` using the `native-mcp` skill, or
...
### WHOX cannot see the deployed server

The server-building part may be correct while the WHOX config is not. Load the `native-mcp` skill and configure the server in `~/.whox/config.yaml`, then restart WHOX.
